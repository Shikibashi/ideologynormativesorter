...
export const DEFAULT_PRIORITY_PROMPT =
  "How important is the policy or strategy you selected, relative to other changes?";
export const SKIP_SALIENCE_LABEL =
  "Skip rating and exclude this answer from my result";
...
export function presentedResponseOptions(
  question: Question,
  includeRefusal = false,
): PresentedResponseOption[] {
  const options: PresentedResponseOption[] =
    question.responseType === "statementChoice"
      ? (question.statementOptions ?? []).map((option, index) => ({
          value: index,
          label: option.text,
          optionId: option.id,
        }))
      : scaleValues(question.responseType).map((value) => ({
          value,
          label: scaleLabel(question.responseType, value),
        }));
