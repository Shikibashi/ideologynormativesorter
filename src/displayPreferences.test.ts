...
describe("display preference resolution", () => {
  it("uses safe defaults and accepts persisted explicit preferences", () => {
    ...
    expect(
      readAppearance(storage({ "political-judgment-appearance-v1": "light" })),
    ).toBe("light");
    expect(
      readDensity(storage({ "political-judgment-density-v1": "comfortable" })),
    ).toBe("comfortable");
    expect(
      readContrast(storage({ "political-judgment-contrast-v1": "more" })),
    ).toBe("more");
  });
