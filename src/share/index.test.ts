...
describe("encodeAnswers / decodeAnswers", () => {
  ...
  it("rejects missing or mismatched version metadata when compatibility is required", () => {
    ...
    const stale = encodeAnswers(SAMPLE_ANSWERS, {
      bankVersion: "bank-v0",
      scoringVersion: "score-v1",
    });
    const legacy = encodePayload([["q0001", 1]]);

    expect(decodeCompatibleAnswers(compatible, expected)).toEqual(
      SAMPLE_ANSWERS,
    );
...
describe("extractEncodedAnswers", () => {
  it("extracts answers from full URLs, hashes, raw payloads, and composite params", () => {
    ...
    const encoded2 = encodeAnswers({
      q0001: { questionId: "q0001", value: -2 },
    });

    expect(
      extractEncodedAnswers(`https://example.test/path#r=${encoded1}`),
    ).toBe(encoded1);
    expect(extractEncodedAnswers(`#r=${encoded1}`)).toBe(encoded1);
    expect(extractEncodedAnswers(encoded1)).toBe(encoded1);
    expect(extractEncodedAnswers(`#r=${encoded1}&c=${encoded2}`, "r")).toBe(
      encoded1,
    );
    expect(extractEncodedAnswers(`#r=${encoded1}&c=${encoded2}`, "c")).toBe(
      encoded2,
    );
...
describe("readSharedAnswers", () => {
  ...
  it("reads both original and compare answers from a composite hash", () => {
    ...
    expect(readSharedAnswers()).toEqual(SAMPLE_ANSWERS);
    expect(readCompareAnswers()).toEqual({
      q0001: { questionId: "q0001", value: -2 },
    });
  });
