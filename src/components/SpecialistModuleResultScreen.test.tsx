...
describe("SpecialistModuleResultScreen", () => {
  it("discloses the historical and source boundary of a mutualism family-level comparison", () => {
    ...
    expect(
      view.getByText(/does not identify Proudhonian, Tuckerite, Labadie-line/i),
    ).toBeInTheDocument();

    fireEvent.click(view.getByText("Scope and limitations"));
    expect(
      view.getByText(/Joseph \(Jo\) and Laurance Labadie/i),
    ).toBeInTheDocument();
    expect(
      view.getByText(
        /C4SS is a contemporary left-market-anarchist organization/i,
      ),
    ).toBeInTheDocument();
    ...
    expect(
      view.getByRole("link", {
        name: "Pierre-Joseph Proudhon’s Mutualist Social Science",
      }),
    ).toBeInTheDocument();
    expect(
      view.getByRole("link", { name: "What Is C4SS?" }),
    ).toBeInTheDocument();
    expect(
      view.getByText(/do not validate this experimental comparison/i),
    ).toBeInTheDocument();
  });
  ...
  it("does not display a low-coverage specialist comparison as a public result", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Individualist Anarchism" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText(
        /no candidate profile met the experimental display threshold/i,
      ),
    ).toBeInTheDocument();
  });
});
