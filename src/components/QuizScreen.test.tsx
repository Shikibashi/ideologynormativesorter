...
describe("QuizScreen descriptive questions", () => {
  it("keeps empirical context collapsed and states that sources do not supply the answer", () => {
        ...
        questions={[
          {
            ...descriptiveChoice,
            evidenceNote:
              "Scope to a named institution and observable outcome.",
            sources: [
              {
    ...
    expect(
      screen.getByText(/do not determine how you should answer/i),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /a public source/i }),
  ...
  it("collapses empirical context when advancing to the next sourced item", () => {
    ...
    render(
      <QuizScreen
        questions={[
          ...
          {
            ...sourcedQuestion,
            id: "second-sourced-item",
            prompt: "Does the second empirical claim fit the evidence?",
          },
        ]}
        onComplete={vi.fn()}
      />,
    ...
    fireEvent.click(screen.getByText("Context and sources"));
    expect(
      screen.getByText("Context and sources").closest("details"),
    ).toHaveAttribute("open");
    fireEvent.click(screen.getByRole("button", { name: /i don't know/i }));

    expect(
      screen.getByText("Does the second empirical claim fit the evidence?"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Context and sources").closest("details"),
  ...
  it("offers I do not know even when the data item omits allowDontKnow", () => {
    ...
    render(
      <QuizScreen questions={[descriptiveChoice]} onComplete={onComplete} />,
    );
  ...
  it("uses a neutral fallback confidence prompt for descriptive choices", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: /institutions usually perform well/i,
      }),
    );

    expect(
      screen.getByText("How confident are you in the answer you just gave?"),
    ).toBeInTheDocument();
  ...
  it("states that skipping a rating excludes the substantive answer from the result", () => {
    ...
    expect(
      screen.getByText(
        /skipping the rating excludes the answer from your result/i,
      ),
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", {
...
describe("QuizScreen cross-layer context", () => {
  it("keeps normative context collapsed and labels sources as background", () => {
        ...
        questions={[
          {
            ...prescriptiveChoice,
            contextNote:
              "This context distinguishes a contested strategy from one fixed institutional blueprint.",
            sources: [
              {
                title: "A public context source",
                publisher: "Example Institute",
    ...
    expect(
      screen.getByRole("link", { name: /a public context source/i }),
    ).toHaveAttribute("href", "https://example.com/context");
  });
});
...
describe("QuizScreen research nonresponse", () => {
  it("offers a distinct refusal only when research mode enables it", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /prefer not to answer/i }),
    ).not.toBeInTheDocument();

    rerender(
      <QuizScreen
        questions={[descriptiveChoice]}
        onComplete={onComplete}
        allowRefusal
      />,
    );
    fireEvent.click(
      screen.getByRole("button", { name: /prefer not to answer/i }),
    );
    expect(onComplete).toHaveBeenCalledWith({
      "test-descriptive-choice": {
        questionId: "test-descriptive-choice",
...
describe("QuizScreen prescriptive follow-up", () => {
  it("asks about the selected policy rather than the importance of the question", () => {
    render(
      <QuizScreen questions={[prescriptiveChoice]} onComplete={vi.fn()} />,
    );
