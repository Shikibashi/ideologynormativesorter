...
function drawCompassPoint(
  ...
  context.fillRect(px - 6, py - 6, 12, 12);
  context.strokeRect(px - 6, py - 6, 12, 12);

  context.fillStyle = text;
  context.font = `700 ${microSize}px ${uiFont}`;
  context.textAlign = "center";
  context.fillText(label, px, py - 12);
}
...
function drawCompassGrid(
  ...
  for (const value of [-0.5, 0.5]) {
    ...
    context.lineTo(px, size - pad);
    context.stroke();
    context.beginPath();
    context.moveTo(pad, py);
    context.lineTo(size - pad, py);
    context.stroke();
  }

  context.setLineDash([]);
  context.strokeStyle = strongGrid;
  context.beginPath();
  context.moveTo(cx, pad);
  context.lineTo(cx, size - pad);
  context.stroke();
  context.beginPath();
  context.moveTo(pad, cy);
  context.lineTo(size - pad, cy);
  context.stroke();
}
    ...
    const draw = () => {
      ...
      const uiFont =
        getComputedStyle(document.documentElement)
          .getPropertyValue("--ecw-font-ui")
          .trim() || 'Verdana, "DejaVu Sans", Tahoma, sans-serif';
      const systemFont =
        getComputedStyle(document.documentElement)
          .getPropertyValue("--ecw-font-system")
          .trim() || '"Courier New", "Liberation Mono", monospace';
      ...
      canvas.dataset.ecwDrawCount = String(
        Number(canvas.dataset.ecwDrawCount ?? 0) + 1,
      );
      ...
      drawCompassGrid(ctx, size, pad, cx, cy, plotRadius, grid, strongGrid);
  ...
  return (
    <div className="compass-plot">
      ...
      <p className="muted" style={{ fontSize: "0.8rem" }}>
        Horizontal: economic left to economic right. Vertical: libertarian to
        authoritarian.
      </p>
    </div>
  );
