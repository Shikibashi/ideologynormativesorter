...
export function AxisBar({ axis, score, result }: AxisBarProps) {
  ...
  const salienceLabel =
    axis.layer === "descriptive"
      ? "confidence"
      : axis.layer === "prescriptive"
        ? "priority"
        : null;
  ...
  return (
    <div className="axis-bar">
      <div className="axis-bar-header">
        ...
        <span className="axis-bar-position muted">
          {score.itemCount === 0
            ? "unmeasured"
            : axisPositionLabel(score.normalized, axis)}
        </span>
        {rel && (
          <span className={`reliability ${rel.band}`}>
            {coverageLabel(rel.band)}
          </span>
        )}
      </div>
      <div className="axis-bar-track" aria-hidden="true">
        ...
        {score.itemCount > 0 && (
          <div className="axis-bar-marker" style={{ left: `${percent}%` }} />
        )}
      </div>
      ...
      {showWhy && contribs.length > 0 && (
        <ul className="why-list">
          {contribs.slice(0, 3).map((c: Contribution) => (
            <li key={c.questionId}>
              {c.prompt.slice(0, 80)}... → toward{" "}
              {c.toward === "positive" ? axis.positivePole : axis.negativePole}{" "}
              ({c.layer})
            </li>
          ))}
        </ul>
