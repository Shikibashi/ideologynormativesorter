...
describe('Philosophy Explorer evidence selection', () => {
  it('does not present an opposite-direction axis as philosophy alignment', () => {
    const rows = buildPhilosophyRows(
      ...
      [labelWithInfluence({ centroid: 0.5, descriptivePhilosophies: ['Classical Liberalism'] })],
      [descriptiveAxis],
    )
  ...
  it('honors the label layer classification instead of inferring a layer from affectedAxes', () => {
    const rows = buildPhilosophyRows(
      ...
      [labelWithInfluence({ centroid: 0.5, normativePhilosophies: ['Classical Liberalism'] })],
      [descriptiveAxis],
    )
  ...
  it('uses layer-specific Marxism explanations', () => {
    const normative = philosophyOverview('Marxism', 'normative' satisfies Layer)
    const descriptive = philosophyOverview('Marxism', 'descriptive' satisfies Layer)
