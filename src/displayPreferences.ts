...
export function readAppearance(storage?: StorageReader): Appearance {
  const stored = safelyRead(storage, APPEARANCE_STORAGE_KEY)
...
export function readContrast(storage?: StorageReader): Contrast {
  ...
  return stored === 'standard' || stored === 'more' || stored === 'automatic' ? stored : 'automatic'
}
...
export function resolveAutomaticDensity(capabilities: InputCapabilities): ResolvedDensity {
  if (capabilities.inlineSize < 720) return 'comfortable'
  if (capabilities.anyFine || capabilities.anyHover) return 'compact'
  return capabilities.coarse || capabilities.noHover ? 'comfortable' : 'compact'
}

export function resolveDensity(density: Density, capabilities: InputCapabilities): ResolvedDensity {
  return density === 'automatic' ? resolveAutomaticDensity(capabilities) : density
}

export function resolveContrast(contrast: Contrast, systemMore: boolean): ResolvedContrast {
  return contrast === 'automatic' ? systemMore ? 'more' : 'standard' : contrast
}

export function browserCapabilities(inlineSize = window.innerWidth): InputCapabilities {
