import type { Answer, AnswerMap, Axis, AxisId, IdeologyLabel, Question } from '../types'
import { buildResultProfile } from '../scoring'

export type EstimateStatus = 'estimated' | 'insufficient-data' | 'not-applicable'
...
function weightForAxis(question: Question, axisId: AxisId): number | null {
  return question.axisWeights.find((weight) => weight.axisId === axisId)?.weight ?? null
}

function orientedItemScore(question: Question, answer: Answer | undefined, axisId: AxisId): number | null {
  if (!answer || answer.value === 'dont_know' || typeof answer.value !== 'number') return null
...
function eligibleItems(questions: Question[], axisId: AxisId): Question[] {
  return questions.filter((question) =>
    ...
    && question.axisWeights.some((weight) => weight.axisId === axisId && weight.weight !== 0),
  )
}
...
function completeCaseMatrix(
  ...
  for (const record of records.filter((candidate) => candidate.administration === 'test')) {
    const values = items.map((question) => orientedItemScore(question, record.answers[question.id], axisId))
...
export function cronbachAlpha(matrix: number[][]): number | null {
  ...
  return (itemCount / (itemCount - 1)) * (1 - itemVariances.reduce((sum, variance) => sum + variance, 0) / totalVariance)
}

function alphaEstimate(matrix: number[][], itemCount: number, minimumCases: number): NumericEstimate {
  if (itemCount < 2) {
    return { status: 'not-applicable', n: matrix.length, itemCount, reason: 'At least two common-scale items are required.' }
  }
  ...
  if (value === null) {
    return { status: 'not-applicable', n: matrix.length, itemCount, reason: 'The score has zero variance or cannot be estimated.' }
  }
  return { status: 'estimated', value, n: matrix.length, itemCount, reason: 'Cronbach alpha over complete, directionally aligned Likert responses.' }
}

function splitHalfEstimate(matrix: number[][], itemCount: number, minimumCases: number): NumericEstimate {
  if (itemCount < 4) {
    return { status: 'not-applicable', n: matrix.length, itemCount, reason: 'At least four items are required for an odd-even split.' }
  }
  if (matrix.length < minimumCases) {
    return {
      status: 'insufficient-data',
  ...
  const odd = matrix.map((row) => mean(row.filter((_, index) => index % 2 === 0)))
  const even = matrix.map((row) => mean(row.filter((_, index) => index % 2 === 1)))
  const correlation = pearsonCorrelation(odd, even)
  if (correlation === null || correlation <= -1) {
    return { status: 'not-applicable', n: matrix.length, itemCount, reason: 'Half scores have zero variance or an undefined correlation.' }
  }
  const spearmanBrown = (2 * correlation) / (1 + correlation)
  return { status: 'estimated', value: spearmanBrown, n: matrix.length, itemCount, reason: 'Odd-even correlation with Spearman-Brown correction.' }
}

function itemTotalEstimates(matrix: number[][], items: Question[], minimumCases: number): ItemTotalEstimate[] {
  return items.map((item, column) => {
    ...
    const remainder = matrix.map((row) => row.reduce((sum, value, index) => index === column ? sum : sum + value, 0))
    const correlation = pearsonCorrelation(itemValues, remainder)
    if (correlation === null) {
      return { questionId: String(item.id), status: 'not-applicable', n: matrix.length, reason: 'Item or remainder score has zero variance.' }
    }
    return {
      questionId: String(item.id),
      status: 'estimated',
      correlation,
      n: matrix.length,
      reason: 'Corrected item-total correlation excluding this item from the total.',
    }
  })
}

function axisScore(record: ValidationResponse, items: Question[], axisId: AxisId): number | null {
  const values = items.flatMap((question) => {
    const value = orientedItemScore(question, record.answers[question.id], axisId)
...
function testRetestEstimate(
  ...
  const tests = new Map(records.filter((record) => record.administration === 'test').map((record) => [record.respondentId, record]))
  const retests = new Map(records.filter((record) => record.administration === 'retest').map((record) => [record.respondentId, record]))
  const first: number[] = []
  const second: number[] = []
  ...
  if (correlation === null) {
    return { status: 'not-applicable', n: first.length, itemCount: items.length, reason: 'Test or retest scores have zero variance.' }
  }
  return { status: 'estimated', value: correlation, n: first.length, itemCount: items.length, reason: 'Pearson correlation across matched test-retest respondents.' }
}
...
export function directionalBalance(questions: Question[], axes: Axis[]): DirectionalBalance[] {
  return axes.map((axis) => {
    let positiveItems = 0
    let negativeItems = 0
    let ipsativeOptions = 0
    for (const question of questions.filter((item) => item.active !== false && item.reviewStatus !== 'needs-rewrite')) {
      if (question.responseType === 'statementChoice') {
        for (const option of question.statementOptions ?? []) {
          if (option.axisWeights.some((weight) => weight.axisId === axis.id && weight.weight !== 0)) ipsativeOptions += 1
        }
        continue
      }
      const weight = question.axisWeights.find((candidate) => candidate.axisId === axis.id)?.weight
    ...
    return {
      ...
      negativeItems,
      ipsativeOptions,
      imbalance: directionalTotal === 0 ? 1 : Math.abs(positiveItems - negativeItems) / directionalTotal,
    }
  })
}

export function sourceCoverage(questions: Question[]): SourceCoverageReport {
  const descriptive = questions.filter((question) => question.active !== false && question.layer === 'descriptive')
  const sourcedItems = descriptive.filter((question) => (question.sources?.length ?? 0) > 0).length
  const operationalizedItems = descriptive.filter((question) => Boolean(question.evidenceNote?.trim())).length
  return {
    descriptiveItems: descriptive.length,
    sourcedItems,
    operationalizedItems,
    sourcedRate: descriptive.length === 0 ? 0 : sourcedItems / descriptive.length,
    operationalizedRate: descriptive.length === 0 ? 0 : operationalizedItems / descriptive.length,
  }
}
...
function selfLabelEstimate(
  ...
  const usable = records.filter((record) => record.administration === 'test' && record.selfLabelId && labels.some((label) => label.id === record.selfLabelId))
  ...
  return {
    ...
    reason: 'Agreement with optional ideology self-identification captured before results. This is criterion evidence, not proof of validity.',
  }
}
...
export function analyzePsychometricStudy(
  ...
  const testRecords = records.filter((record) => record.administration === 'test')
  const retestRecords = records.filter((record) => record.administration === 'retest')
  const uniqueRespondents = new Set(records.map((record) => record.respondentId)).size
  ...
  const axisReports = axes.map((axis) => {
    ...
    return {
      ...
      cronbachAlpha: alphaEstimate(matrix, items.length, resolved.minimumCompleteCases),
      splitHalfReliability: splitHalfEstimate(matrix, items.length, resolved.minimumCompleteCases),
      testRetestCorrelation: testRetestEstimate(records, items, axis.id, resolved.minimumRetestPairs),
      itemTotalCorrelations: itemTotalEstimates(matrix, items, resolved.minimumCompleteCases),
      missingResponseRate: denominator === 0 ? 0 : distribution.missing / denominator,
      floorRate: distribution.observed === 0 ? 0 : distribution.floor / distribution.observed,
      ceilingRate: distribution.observed === 0 ? 0 : distribution.ceiling / distribution.observed,
    }
  })

  const excludedQuestionIds = questions
    .filter((question) => question.reviewStatus === 'needs-rewrite' || question.responseType === 'statementChoice')
    .map((question) => String(question.id))

  const status = uniqueRespondents === 0
    ? 'not-collected'
  ...
  return {
    ...
    selfLabelConcordance: selfLabelEstimate(records, questions, axes, labels, resolved.minimumCriterionCases),
    excludedQuestionIds,
    notes: [
