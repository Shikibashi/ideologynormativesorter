...
describe("wave partition", () => {
  it("partitions effective-active main into chunks of 40 (last may be smaller)", () => {
    ...
    expect(main.length).toBe(
      Math.ceil(WP0_FREEZE.effectiveActiveQuestionCount / 40),
    );
    expect(main.slice(0, -1).every((w) => w.subjectIds.length === 40)).toBe(
      true,
    );
  ...
  it("partitions labels into chunks of 8 with family ordering", () => {
    ...
    expect(waves.flatMap((w) => w.subjectIds)).toHaveLength(
      WP0_FREEZE.labelCount,
    );
    expect(waves.slice(0, -1).every((w) => w.subjectIds.length === 8)).toBe(
      true,
    );
