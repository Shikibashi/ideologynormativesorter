...
export function buildContributionRecords(
  ...
  for (const question of questions) {
    ...
    const rationale =
      options.rationales?.[question.id] ?? defaultRationale
    ...
    for (const response of expandSelectableResponses(question)) {
      for (const axisWeight of response.axisWeights) {
        const id = contributionId(question.id, response.responseKey, axisWeight.axisId)
        const signed = signedContribution(question, response.responseValue, axisWeight.weight)
