...
describe("separability.policy", () => {
  it("no active label has an applied reject-forced-spread violation", () => {
    ...
    const violations = findings.filter(
      (f) =>
        ...
        f.lifecycle === "applied" &&
        f.subjectIds.some((id) =>
          activeLabelIds.has(id as (typeof labels)[number]["id"]),
        ),
    );
  ...
  it("near-duplicate findings exist when pairs are closer than 0.35", () => {
    ...
    const nearDupFindings = buildNearDuplicateFindings(
      separabilityDiagnostics,
    ).filter((f) => f.issueClass === "near-duplicate-centroid");
    ...
    for (const diagnostic of nearDupPairs) {
      ...
      expect(
        hit,
        `missing finding for ${diagnostic.diagnosticId}`,
      ).toBeDefined();
      expect(hit!.lifecycle).toBe("proposed");
      expect(["park-separability", "merge"]).toContain(
        hit!.proposedDisposition,
      );
