...
export function formatPerspectives(
  ...
  return {
    sympathetic: {
      text: `Sympathetic reading: the ${axisId} coordinate for ${label.name} encodes a coherent directional emphasis implied by the label's definition and named philosophy influences.`,
    },
    critical: {
      text: `Critical reading: no family encyclopedia assigns the exact ${axisId} numeric score used for ${label.name}; treating the coordinate as externally validated would overclaim evidence strength.`,
    },
    neutral: {
      text: `Neutral framing: the ${axisId} value is a catalog reference placement for matching distance — model-internal, researched for consistency with the definition, and awaiting qualified-expert review.`,
    },
  };
}
