...
describe("releaseGate", () => {
  it("latest summary fingerprints match live bank + scoring versions", () => {
    ...
    expect(summary.generatedFrom.scoringVersion).toBe(
      WP0_FREEZE.versions.resultScoring,
    );
  });
  ...
  it("releaseGate() fails overall while expert remains non-pass (truthful in-review)", () => {
    ...
    expect(result.failures.some((f) => f.includes("unresolved active"))).toBe(
      false,
    );
  });
  ...
  it("textual in-review does not block release when expert is pass", () => {
    ...
    const textual = withExpertPass.gateStatuses.find(
      (g) => g.gate === "textual",
    );
    expect(textual?.status).toBe("in-review");
    ...
    expect(
      result.failures.some((f) => f.toLowerCase().includes("textual")),
    ).toBe(false);
  });
