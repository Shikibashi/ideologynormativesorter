...
import { analyzePsychometricStudy, type ValidationResponse } from '../psychometrics'
...
const ALLOWED_PHRASES = ['pilot', 'insufficient data', 'non-empirical'] as const
...
describe('copy overclaim guards', () => {
  it('MethodologyScreen and ResultsScreen avoid forbidden accuracy overclaim phrases', () => {
    for (const path of COPY_SOURCES) {
      ...
      for (const phrase of FORBIDDEN_OVERCLAIM_PHRASES) {
        expect(lower.includes(phrase.toLowerCase()), `${path} contains forbidden phrase: ${phrase}`).toBe(
          false,
        )
      }
  ...
  it('empty psychometric study yields insufficient-data style status, not high confidence', () => {
    ...
    expect(estimatedStatuses.every((status) => status !== 'estimated')).toBe(true)
