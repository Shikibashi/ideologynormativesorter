...
function answersFor(value: number): AnswerMap {
  return Object.fromEntries(
    items.map((item) => [item.id, { questionId: item.id, value }]),
  );
}
...
describe("analyzePsychometricStudy", () => {
  it("reports not-collected instead of fabricating estimates", () => {
    const report = analyzePsychometricStudy([], items, [axis], [], {
      minimumCompleteCases: 10,
    });
    expect(report.status).toBe("not-collected");
    expect(report.axisReports[0].cronbachAlpha.status).toBe(
      "insufficient-data",
    );
      ...
      { ...items[0], id: "rewrite", reviewStatus: "needs-rewrite" },
      {
        ...
        statementOptions: [
          ...
          },
          {
            id: "c",
            text: "C",
            axisWeights: [{ axisId: axis.id, weight: 0.2 }],
          },
        ],
      },
    ];
    const report = analyzePsychometricStudy(
      [],
      [...items, ...excluded],
      [axis],
      [],
