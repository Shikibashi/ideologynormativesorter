...
export function savePendingResearchRecord(record: PendingResearchRecord): SaveResult {
  try {
    localStorage.setItem(PENDING_RESEARCH_KEY, JSON.stringify(record))
    return { saved: true }
  } catch {
    return { saved: false, reason: 'The completed research record could not be saved in this browser.' }
  }
}
...
function isQuizSave(value: unknown): value is QuizSave {
  ...
  const completedAtIsValid = candidate.completedAt === undefined
    || (typeof candidate.completedAt === 'string' && Number.isFinite(Date.parse(candidate.completedAt)))
  const completedAnswersAreComplete = candidate.completedAt === undefined
    || (Array.isArray(candidate.questions)
      && candidate.questions.every((question) => candidate.answers?.[question.id] !== undefined))
  return (
    Array.isArray(candidate.questions) &&
    ...
    (candidate.startedAt === undefined || (typeof candidate.startedAt === 'string' && Number.isFinite(Date.parse(candidate.startedAt)))) &&
    completedAtIsValid &&
    completedAnswersAreComplete &&
    (candidate.research === undefined || isResearchSaveContext(candidate.research)) &&
...
function isResearchSaveContext(value: unknown): boolean {
  ...
  return typeof candidate.participantId === 'string'
    ...
    && (candidate.administration === 'test' || candidate.administration === 'retest')
    ...
    && (candidate.requestedItemCount === null
      || (Number.isInteger(candidate.requestedItemCount) && candidate.requestedItemCount > 0))
}

function isPendingResearchRecord(value: unknown): value is PendingResearchRecord {
  if (!value || typeof value !== 'object') return false
  const candidate = value as Partial<PendingResearchRecord>
  if (!candidate.submission || typeof candidate.submission !== 'object') return false
  ...
  return typeof submission.schemaVersion === 'string'
    ...
    && (status.status === 'submitted' || status.status === 'export-only' || status.status === 'failed')
}
