...
export function axisPositionLabel(
  value: number,
  axis: Pick<Axis, "negativePole" | "positivePole">,
): string {
...
export function comparisonStabilityLabel(
...
export function idealGapLabel(gap: number): string {
  ...
  if (magnitude < 0.2)
    return "Small difference between ideal and current-condition answers";
  if (magnitude < 0.4)
    return "Noticeable difference between ideal and current-condition answers";
