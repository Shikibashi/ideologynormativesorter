...
describe('twenty-sixth editorial pass', () => {
  it('versions and applies each source-bounded correction', () => {
    expect(QUESTION_BANK_VERSION).toContain(EDITORIAL_TWENTY_SIXTH_PASS_VERSION)
    ...
    for (const [id, correction] of Object.entries(descriptiveConstructCorrectionsById)) {
      ...
      if (correction.prompt) expect(question?.prompt, id).toBe(correction.prompt)
    }
  })
  ...
  it('removes unsupported institutional and directional implications', () => {
    ...
    expect(questionById.get('q0308')?.prompt).toMatch(/competitive conditions/i)
