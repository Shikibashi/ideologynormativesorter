...
describe("ideology taxonomy", () => {
  ...
  it("keeps final religious legal authority off the ordinary modifier path", () => {
    ...
    expect(
      publicCatalogLabels.find((label) => label.id === "theocrat")?.taxonomy
        .measurementStatus,
  ...
  it("covers the major political families in the primary pool", () => {
    ...
    for (const family of MAJOR_PRIMARY_FAMILIES) {
      expect(
        families.has(family),
        `primary scoring pool is missing ${family}`,
      ).toBe(true);
    }
  });
  ...
  it("separates right-libertarian lineage from market-anarchist catalog entries", () => {
    ...
    for (const labelId of LIBERAL_LIBERTARIAN_LABEL_IDS) {
      expect(
        labelById.get(labelId)?.family,
        `${labelId} should be grouped under liberal`,
      ).toBe("liberal");
    }
    ...
    for (const labelId of ANARCHIST_MARKET_LABEL_IDS) {
      expect(
        labelById.get(labelId)?.family,
        `${labelId} should be grouped under anarchist`,
      ).toBe("anarchist");
    }

    expect(labelById.get("libertarian-socialism")?.family).toBe("socialist");
    expect(labelById.get("left-wing-market-anarchism")?.family).toBe(
      "anarchist",
    );
    ...
    expect(
      publicCatalogLabels.find(
        (label) => label.id === "market-right-libertarianism",
      )?.name,
    ).toBe("Right-Libertarianism");
    expect(labelById.get("libertarian-municipalism")?.family).not.toBe(
      "liberal",
    );
  });

  it("uses typed relations instead of false parentage for contested anarchist and Marxian boundaries", () => {
    const anarchoCapitalistRelations =
      taxonomyForLabel("anarcho-capitalist")?.relations ?? [];
    expect(anarchoCapitalistRelations).toEqual(
    ...
    expect(taxonomyForLabel("marxian-socialism")?.relations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "contrasts_with",
    ...
    expect(taxonomyForLabel("mutualist")?.relations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "overlaps_with",
    ...
    expect(taxonomyForLabel("individualist-anarchism")?.relations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "influenced_by",
          labelId: "mutualist",
    ...
    expect(
      taxonomyForLabel("individualist-anarchism")?.relations.find(
        (relation) => relation.labelId === "mutualist",
      )?.note,
  ...
  it("keeps technocratic centralism a narrow compound rather than a child of generic expertise", () => {
    ...
    expect(technocraticCentralism?.relations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "requires",
  ...
  it("keeps adjacent anarchist and anti-state labels from inheriting narrower doctrines as required subtypes", () => {
    ...
    expect(
      labelById.get("left-wing-market-anarchism")?.prescriptivePhilosophies,
    ).not.toContain("Mutualism");
    expect(labelById.get("individualist-anarchism")?.philosophies).not.toEqual(
      expect.arrayContaining(["Stirnerism", "Egoism"]),
    );
    expect(labelById.get("individualist-anarchism")?.subTheories).not.toContain(
      "Post-Left Anarchism",
    );
    expect(labelById.get("voluntaryism")?.aliases ?? []).not.toContain(
      "Contractual Anarchism",
    );
    expect(labelById.get("stirnerism")?.philosophies).not.toContain("Nihilism");
    expect(labelById.get("stirnerism")?.subTheories).toEqual([]);
    expect(labelById.get("techno-anarchism")?.philosophies).not.toContain(
      "Cyber-Libertarianism",
    );
    expect(labelById.get("techno-anarchism")?.subTheories).toEqual([]);
    expect(labelById.get("queer-anarchism")?.subTheories).not.toContain(
      "Gender Abolition",
    );
    expect(
      labelById.get("anarcha-feminism")?.prescriptivePhilosophies,
    ).toContain("Anarcha-Feminism");
  });
  ...
  it("keeps socialist lineages broad without forcing adjacent traditions into Marxism or one generic subfamily", () => {
    ...
    expect(labelById.get("council-communist")?.subTheories).not.toContain(
      "Luxemburgism",
    );
    expect(labelById.get("christian-socialism")?.subfamily).toBe(
      "religious-socialist",
    );
    expect(labelById.get("utopian-socialism")?.subfamily).toBe(
      "early-socialist",
    );
    expect(labelById.get("marxist-leninist")?.subTheories).not.toContain(
      "Titoism",
    );
    expect(labelById.get("maoism")?.philosophies).toContain("Maoism");
    expect(labelById.get("maoism")?.prescriptivePhilosophies).toContain(
      "Maoism",
    );
    expect(labelById.get("trotskyism")?.philosophies).toContain("Trotskyism");
    expect(labelById.get("trotskyism")?.prescriptivePhilosophies).toContain(
      "Trotskyism",
    );
    expect(labelById.get("guild-socialism")?.philosophies).not.toContain(
      "Distributism",
    );
    expect(
      labelById.get("guild-socialism")?.normativePhilosophies,
    ).not.toContain("Distributism");
  ...
  it("keeps conservative traditions distinct from thin positions, influences, and catalog-specific hybrids", () => {
    ...
    expect(
      labelById.get("paleoconservatism")?.prescriptivePhilosophies,
    ).toContain("Paleoconservatism");
    expect(
      labelById.get("one-nation-conservatism")?.subTheories,
    ).toBeUndefined();
    ...
    expect(
      labelById.get("social-conservatism")?.descriptivePhilosophies,
    ).toEqual([]);
    ...
    expect(
      labelById.get("liberal-conservatism")?.descriptivePhilosophies,
    ).toEqual([]);
    expect(labelById.get("conservative-liberalism")?.usageNote).toMatch(
      /not a universal taxonomy/,
    );
    expect(labelById.get("liberal-conservatism")?.usageNote).toMatch(
      /not a universal taxonomy/,
    );
  });
  ...
  it("keeps nationalist membership and sovereignty traditions distinct from required subtypes and explanatory theories", () => {
    ...
    expect(labelById.get("civic-nationalist")?.normativePhilosophies).toContain(
      "Civic Nationalism",
    );
    expect(labelById.get("indigenism")?.prescriptivePhilosophies).toContain(
      "Indigenism",
    );
    expect(labelById.get("hindutva")?.prescriptivePhilosophies).toContain(
      "Hindutva",
    );
    expect(
      labelById.get("religious-nationalism")?.prescriptivePhilosophies,
    ).toContain("Religious Nationalism");
    expect(labelById.get("zionism")?.prescriptivePhilosophies).toContain(
      "Zionism",
    );
    expect(
      labelById.get("left-wing-nationalism")?.prescriptivePhilosophies,
    ).toContain("Anti-Colonialism");
    expect(
      labelById.get("expansionist-nationalism")?.philosophies,
    ).not.toContain("Realism");
    expect(
      labelById.get("expansionist-nationalism")?.philosophies,
    ).not.toContain("Social Darwinism");
    expect(
      labelById.get("expansionist-nationalism")?.descriptivePhilosophies,
    ).toEqual([]);
    expect(labelById.get("separatist-nationalism")?.usageNote).toMatch(
      /does not by itself establish a right to unilateral secession/,
    );
  });
  ...
  it("does not score policy proposals, futurist concepts, or governance mechanisms as primary ideologies", () => {
    for (const labelId of NON_IDEOLOGY_ENDPOINTS) {
      expect(
        roleForLabel(labelId),
        `${labelId} should not be primary`,
      ).not.toBe("primary");
      expect(primaryScoringLabels.some((label) => label.id === labelId)).toBe(
        false,
      );
    }
  });
  ...
  it("only exposes modifiers with declared direct core constructs to ordinary scoring", () => {
    const ordinaryModifierIds = new Set(
    ...
    for (const labelId of MODIFIER_LABEL_IDS) {
      ...
      expect(
        measurement,
        `${labelId} needs a measurement disposition`,
      ).toBeDefined();
      expect(ordinaryModifierIds.has(labelId), labelId).toBe(
        measurement?.availability === "core-construct",
      );
    }
  });
  ...
  it("keeps labels whose defining construct is absent from the core instrument out of ordinary results", () => {
    ...
    expect(PROVISIONAL_SPECIALIST_LABEL_IDS).toEqual(
      expect.arrayContaining([
        "eco-authoritarianism",
        "fascist-authoritarian",
        "welfare-chauvinism",
    ...
    expect(specialistModuleByLabel["religious-nationalism"]).toBe(
      "religious-national-politics-module",
    );
    expect(specialistModuleByLabel.theocrat).toBe(
      "religious-national-politics-module",
    );
  ...
  it("keeps every named high-risk tradition out of ordinary results except gated Marxism-Leninism", () => {
    ...
    for (const labelId of HIGH_RISK_NON_ORDINARY_RESULT_IDS) {
      expect(
        ordinaryResultIds,
        `${labelId} must require a specialist or focused follow-up`,
      ).not.toContain(labelId);
    }
  });
  ...
  it("keeps retired synthetic labels out of both scoring and the public label browser", () => {
    for (const labelId of RETIRED_LABEL_IDS) {
      ...
      expect(publicCatalogLabels.some((label) => label.id === labelId)).toBe(
        false,
      );
    }
  });
  ...
  it("keeps context-only concepts out of scoring while keeping them browsable", () => {
    for (const labelId of CONTEXT_LABEL_IDS) {
      ...
      expect(
        publicCatalogLabels.find((label) => label.id === labelId)?.taxonomy
          .measurementStatus,
      ).toBe("context-only");
    }
  });
  ...
  it("uses the same primary pool for research self-identification as for predicted results", () => {
    expect(researchIdentityLabels.map((label) => label.id)).toEqual(
      primaryScoringLabels.map((label) => label.id),
    );
  });
  ...
  it("requires every non-provisional specialist label to have an existing, substantive depth module", () => {
    const moduleCounts = new Map<string, number>(
      specialistModuleDefinitions.map((module) => [
        module.id,
        module.questions.length,
      ]),
    );
    const provisionalSpecialists = new Set<string>(
      PROVISIONAL_SPECIALIST_LABEL_IDS,
    );
  ...
  it("exposes socialist feminism only through its construct-matched specialist module", () => {
    ...
    expect(
      primaryScoringLabels.some((label) => label.id === "socialist-feminism"),
    ).toBe(false);
    expect(
      publicCatalogLabels.some((label) => label.id === "socialist-feminism"),
    ).toBe(true);
    expect(specialistModuleByLabel["socialist-feminism"]).toBe(
  ...
  it("routes labels according to what the current instrument can actually distinguish", () => {
    ...
    expect(specialistModuleByLabel["liberal-feminism"]).toBe(
      "feminist-faction-module",
    );
    ...
    expect(
      publicCatalogLabels.find((label) => label.id === "market-anarchism")
        ?.family,
    ).toBe("anarchist");
    expect(taxonomyForLabel("market-anarchism")?.parentId).toBeUndefined();
    expect(specialistModuleByLabel["social-anarchism"]).toBe(
  ...
  it("keeps historical aliases and compound profiles analytically compositional", () => {
    ...
    expect(LEGACY_LABEL_DISPOSITIONS["cultural-populism"]).toMatchObject({
      disposition: "keep-retired",
    });
    expect(LEGACY_LABEL_DISPOSITIONS["egalitarian-statist"]).toMatchObject({
      disposition: "keep-retired",
    });
    expect(
      LEGACY_LABEL_DISPOSITIONS["revolutionary-collectivist"],
    ).toMatchObject({ disposition: "keep-retired" });
    expect(
      taxonomyForLabel("decentralist-market-skeptic-of-state"),
    ).toMatchObject({
      legacyDisposition: "split",
      legacyComponents: ["market-liberal", "decentralist-orientation"],
    });
    expect(
      taxonomyForLabel("bright-green-environmentalism")?.relations,
    ).toEqual([
      expect.objectContaining({ type: "alias_of", labelId: "ecomodernist" }),
    ]);
    expect(taxonomyForLabel("social-anarchism")?.relations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "subtype_of",
          labelId: "libertarian-socialism",
        }),
      ]),
    );
    expect(
      taxonomyForLabel("developmental-authoritarianism")?.relations,
    ).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "hybrid_of",
          labelId: "developmentalism",
        }),
        expect.objectContaining({
          type: "requires",
          labelId: "technocratic-orientation",
  ...
  it("does not route world federalism through nationalist discriminators", () => {
    ...
    expect(
      publicCatalogLabels.find((label) => label.id === "world-federalism")
        ?.taxonomy.measurementStatus,
    ).toBe("context-only");
  ...
  it("measures the strongest distinctions between every primary label and its nearest primary neighbor", () => {
    for (const label of primaryScoringLabels) {
      const neighbors = primaryScoringLabels
        ...
        .map((candidate) => ({
          candidate,
          distance: centroidDistance(label.centroid, candidate.centroid),
        }))
      ...
      const strongestDifferences = axes
        .map((axis) => ({
          axisId: axis.id,
          difference: Math.abs(
            label.centroid[axis.id] - nearest!.centroid[axis.id],
          ),
        }))
      ...
      for (const difference of strongestDifferences) {
        const itemCount = questions.filter((question) =>
          question.axisWeights.some(
            (weight) => weight.axisId === difference.axisId,
          ),
        ).length;
        expect(
          itemCount,
