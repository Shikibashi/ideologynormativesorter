...
describe("descriptive evidence context", () => {
  it("versions the bank and attaches every reviewed evidence record to an active descriptive item", () => {
    ...
    for (const [id, evidence] of Object.entries(descriptiveEvidenceById)) {
      ...
      expect(
        question,
        `${id} evidence references a missing item`,
      ).toBeDefined();
      expect(
        question!.active,
        `${id} evidence references an inactive item`,
      ).not.toBe(false);
      expect(question!.layer).toBe("descriptive");
      const laterRewrite =
        twentiethPassRewritesById[id] ??
      ...
      expect(question!.evidenceNote).toBe(
        laterRewrite?.evidenceNote ?? evidence.evidenceNote,
      );
      const expectedSources = laterRewrite
        ? laterRewrite.sourceIds.map(
            (sourceId) => questionContextSources[sourceId],
          )
        : evidence.sources;
      expect(question!.sources?.slice(0, expectedSources.length)).toEqual(
        expectedSources,
      );
  ...
  it("preserves first-pass evidence while the effective bank reaches complete active-item coverage", () => {
    ...
    const sourced = activeDescriptive.filter(
      (question) => (question.sources?.length ?? 0) > 0,
    );
    const operationalized = activeDescriptive.filter((question) =>
