...
describe('fourth descriptive evidence pass', () => {
  it('attaches evidence to every new descriptive confidence item', () => {
    expect(QUESTION_BANK_VERSION).toContain(DESCRIPTIVE_EVIDENCE_FOURTH_PASS_VERSION)

    for (const [id, evidence] of Object.entries(descriptiveEvidenceFourthPassById)) {
      const question = questionById.get(id)
      expect(question?.active, `${id} should not be inactive`).not.toBe(false)
      expect(question?.layer, `${id} should remain descriptive`).toBe('descriptive')
