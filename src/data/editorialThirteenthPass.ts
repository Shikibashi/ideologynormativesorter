 ...
 */
export const thirteenthPassRewritesById: Readonly<Record<string, ThirteenthPassRewrite>> = {
  q0093: {
    prompt: 'An ideal labor regime would extend basic legal protection to workers regardless of whether they are classified as employees, independent contractors, or members of a worker cooperative.',
    rationale: 'Separate worker-status classifications from unions and partnerships, which are collective or firm-level institutions rather than parallel employment statuses. The ILO supports protecting workers across contractual arrangements while preserving distinctions in legal status and bargaining institutions.',
  ...
  q0173: {
    prompt: 'An ideal rights regime would protect core civil liberties by default, requiring any restriction on expression, association, religion, encryption, or due process to satisfy a specific, publicly justified legal test.',
    rationale: 'Turn the list of rights into one coherent default-protection principle and state that restrictions require justification. The ICCPR supplies the legal-rights frame, while cryptographic standards provide the narrower technical context for encryption; the item does not claim that all five examples share one limitation test.',
  ...
  q0191: {
    prompt: 'In plea-bargaining systems, the prospect of a substantially harsher outcome after trial can pressure some defendants—including some who maintain innocence—to accept a plea, although the size and direction of the effect vary by case and are difficult to estimate.',
    rationale: 'Replace the universal-sounding claim about innocent or overcharged defendants surrendering trial rights with a scoped pressure mechanism. Empirical research supports concern about innocent pleas in some settings but also finds an innocence effect and substantial selection limits, so the item should not imply that most pleas are false or that every plea/trial gap is coercive.',
