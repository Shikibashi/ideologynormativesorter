...
function auditAxisWeights(
  ...
  for (const weight of weights) {
    if (seen.has(weight.axisId))
      problems.push(`duplicate axis reference: ${source} -> ${weight.axisId}`);
    seen.add(weight.axisId);

    if (!isFiniteUnit(weight.weight))
      problems.push(`invalid axis weight: ${source} -> ${weight.axisId}`);
    ...
    if (axis.layer !== layer)
      problems.push(
        `cross-layer axis: ${source} (${layer}) -> ${weight.axisId} (${axis.layer})`,
      );
  }
}
...
export function auditCorpus(input?: Partial<AuditInput>): AuditReport {
  ...
  const activeQuestions = auditInput.questions.filter(
    (q) => q.active !== false,
  );
  const totals = {
  ...
  for (const q of activeQuestions) {
    ...
    if (!knownDomainIds.has(q.domain))
      problems.push(`unknown domain: ${q.id} -> ${q.domain}`);

    if (q.layer === "descriptive" && q.allowDontKnow && !q.confidencePrompt)
      problems.push(`descriptive missing confidencePrompt: ${q.id}`);
    if (q.layer === "prescriptive" && !q.priorityPrompt)
      problems.push(`prescriptive missing priorityPrompt: ${q.id}`);
    ...
    if (q.responseType === "statementChoice") {
      if (!q.statementOptions || q.statementOptions.length === 0)
        problems.push(`statement options missing: ${q.id}`);
      const optionIds = new Set<string>();
      for (const option of q.statementOptions ?? []) {
        if (option.id.trim().length === 0)
          problems.push(`statement option empty id: ${q.id}`);
        if (option.text.trim().length === 0)
          problems.push(`statement option empty text: ${q.id}/${option.id}`);
        if (optionIds.has(option.id))
          problems.push(`duplicate statement option id: ${q.id}/${option.id}`);
  ...
  if (
    layerCounts.normative === 0 ||
    layerCounts.descriptive === 0 ||
    layerCounts.prescriptive === 0
  )
  ...
  for (const label of auditInput.labels) {
    for (const axisId of Object.keys(label.centroid)) {
      if (!axisIds.has(axisId))
        problems.push(`unknown label centroid axis: ${label.id} -> ${axisId}`);
      const value = label.centroid[axisId];
      if (!Number.isFinite(value) || value < -1 || value > 1)
        problems.push(`invalid label centroid value: ${label.id} -> ${axisId}`);
    }
    for (const axis of auditInput.axes) {
      if (label.centroid[axis.id] === undefined)
        problems.push(`label centroid missing axis: ${label.id} -> ${axis.id}`);
    }
  }
  ...
  return {
    ...
    review: {
      corrected: activeQuestions.filter(
        (question) =>
          question.version === "2026-07-semantic-v1" &&
          question.reviewStatus === "approved",
      ).length,
      needsRewrite: activeQuestions.filter(
        (question) => question.reviewStatus === "needs-rewrite",
      ).length,
      unreviewedMetadata: activeQuestions.filter(
        (question) => question.reviewStatus === undefined,
      ).length,
    },
    problems,
  };
