...
export type AssignedIdeologyAnalyticalScale = Exclude<IdeologyAnalyticalScale, 'nano'>
...
const SOURCES = Object.freeze(IDEOLOGY_SCALE_SOURCES.map((source) => Object.freeze({ ...source })))
 ...
 */
export function ideologyScaleMetadataForLabel(_labelId: string, role: string): IdeologyScaleMetadata {
  const respondentMeasurementScale = ['primary', 'modifier'].includes(role) ? 'micro' as const : null
...
export const ANALYTICAL_SCALE_GUIDANCE: Readonly<Record<IdeologyAnalyticalScale, string>> = {
  macro: 'Canon, doctrine, social order, population, or society-wide institutional pattern.',
  meso: 'Party, movement, organization, coalition, regime project, or public political discourse.',
  micro: 'Individual uptake, local practice, household, or interactional use; the relevant unit depends on the question.',
