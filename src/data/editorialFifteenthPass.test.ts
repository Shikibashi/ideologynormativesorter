...
describe('fifteenth editorial pass', () => {
  it('narrows six high-risk prescriptive items and preserves their layer', () => {
    ...
    for (const [id, rewrite] of Object.entries(fifteenthPassRewritesById)) {
      ...
      expect(question.sources?.map((source) => source.url)).toEqual(
        rewrite.sourceIds.map((sourceId) => questionContextSources[sourceId]?.url),
      )
  ...
  it('removes the earlier compound or overbroad wording', () => {
    expect(questionById.get('q0033')?.prompt).not.toMatch(/land rents|state-created privileges/i)
    expect(questionById.get('q0036')?.prompt).not.toMatch(/small firms|mutual-aid|independent work/i)
    expect(questionById.get('q0193')?.prompt).not.toMatch(/restitution|prevention/i)
    expect(questionById.get('q0354')?.prompt).not.toMatch(/sunset review|competitive alternatives/i)
    expect(questionById.get('q0375')?.prompt).not.toMatch(/warrants|minimization|adversarial/i)
    expect(questionById.get('q0376')?.prompt).not.toMatch(/benefits|policing|immigration/i)
  })
})
