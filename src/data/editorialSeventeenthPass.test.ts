...
import { descriptiveConstructCorrectionsById as v26Corrections, EDITORIAL_TWENTY_SIXTH_PASS_VERSION } from './editorialTwentySixthPass'
import {
...
describe('seventeenth editorial pass', () => {
  it('registers and applies the source-backed descriptive narrowing pass', () => {
    ...
    for (const id of expectedIds) {
      ...
      expect(question.prompt, id).toBe(v26Corrections[id]?.prompt ?? descriptiveConstructCorrectionsById[id]?.prompt ?? laterRewrite?.prompt ?? rewrite.prompt)
      expect(question.evidenceNote, id).toBe(laterRewrite?.evidenceNote ?? rewrite.evidenceNote)
      expect(question.version, id).toBe(v26Corrections[id]
      ...
      expect(
        question.sources?.slice(0, rewrite.sourceIds.length).map((source) => source.url),
        id,
      ).toEqual(rewrite.sourceIds.map((sourceId) => questionContextSources[sourceId].url))
  ...
  it('removes the reviewed overclaiming and rhetorical shortcuts', () => {
    expect(questionById.get('q0012')?.prompt).not.toMatch(/depends less on a single sovereign/i)
    expect(questionById.get('q0027')?.prompt).not.toMatch(/artificial scarcity/i)
    expect(questionById.get('q0171')?.prompt).not.toMatch(/public panic/i)
    expect(questionById.get('q0269')?.prompt).not.toMatch(/administratively invisible/i)
    expect(questionById.get('q0308')?.prompt).not.toMatch(/often favor large incumbents/i)
  })
})
