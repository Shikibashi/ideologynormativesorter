...
describe('fourteenth editorial pass', () => {
  it('keeps the reviewed descriptive items within their source scope', () => {
    ...
    expect(Object.keys(fourteenthPassRewritesById)).toEqual(['q0147', 'q0248', 'q0307', 'q0368'])
    ...
    for (const [id, rewrite] of Object.entries(fourteenthPassRewritesById)) {
      ...
      expect(question.sources?.length).toBeGreaterThanOrEqual(rewrite.sourceIds.length)
  ...
  it('removes the unsupported broad mechanisms from the reviewed prompts', () => {
    expect(questionById.get('q0147')?.prompt).not.toMatch(/increase innovation|shared knowledge/i)
    expect(questionById.get('q0248')?.prompt).not.toMatch(/doctrin|enforceable law|cleric/i)
    expect(questionById.get('q0307')?.prompt).not.toMatch(/reduce harm|victims|pollution/i)
