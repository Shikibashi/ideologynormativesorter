...
const candidateById = new Map(
  feministSpecialistCandidates.map((candidate) => [candidate.id, candidate]),
);
...
describe("feminist breadth module", () => {
  it("keeps the specialist measurement surface isolated from ordinary quiz tiers", () => {
    ...
    expect(
      new Set(feministModuleItems.map((item) => item.question.id)).size,
    ).toBe(feministModuleItems.length);
  ...
  it("measures every specialist construct with multiple independent items", () => {
    for (const constructId of FEMINIST_CONSTRUCT_IDS) {
      ...
      expect(
        coverage,
        `${constructId} has insufficient module coverage`,
      ).toBeGreaterThanOrEqual(2);
    }
  });

  it("keeps the materialist strategy item focused on social reproduction", () => {
    const item = feministModuleItems.find(
      (entry) => entry.question.id === "fm-fem-5",
    );
  ...
  it("keeps specialist candidate prototypes meaningfully separated", () => {
    ...
    for (let i = 0; i < feministSpecialistCandidates.length; i++) {
      for (let j = i + 1; j < feministSpecialistCandidates.length; j++) {
        ...
        const distance = feministConstructDistance(
          left.centroid,
          right.centroid,
        );
    ...
    expect(
      minimumDistance,
      `closest feminist specialist pair: ${closestPair}`,
  ...
  it("keeps specialist traditions out of primary scoring and quarantines the unlisted candidate", () => {
    ...
    expect(candidateById.get("socialist-feminism")?.status).toBe(
      "existing-specialist",
    );
    expect(catalogIds.has("socialist-feminism")).toBe(true);
    expect(primaryIds.has("socialist-feminism")).toBe(false);

    expect(candidateById.get("radical-feminism")?.status).toBe(
      "candidate-specialist",
    );
    expect(
      catalogIds.has("radical-feminism"),
      "radical-feminism must remain browsable while provisional",
    ).toBe(true);
    expect(
      primaryIds.has("radical-feminism"),
      "radical-feminism leaked into the primary pool",
    ).toBe(false);
  });
