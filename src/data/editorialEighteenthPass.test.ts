...
describe('eighteenth editorial pass', () => {
  it('registers and applies each source-backed descriptive rewrite', () => {
    ...
    for (const [id, rewrite] of Object.entries(eighteenthPassRewritesById)) {
      ...
      expect(question.prompt, id).toBe(v26Corrections[id]?.prompt ?? descriptiveConstructCorrectionsById[id]?.prompt ?? rewrite.prompt)
  ...
  it('removes directional and motive-imputing shortcuts', () => {
    expect(questionById.get('q0050')?.prompt).not.toMatch(/often end up using/i)
    expect(questionById.get('q0089')?.prompt).not.toMatch(/protects incumbent workers more than consumers/i)
    expect(questionById.get('q0108')?.prompt).not.toMatch(/neighborhood preferences|outsiders/i)
    expect(questionById.get('q0128')?.prompt).not.toMatch(/already hold financial assets/i)
    expect(questionById.get('q0130')?.prompt).not.toMatch(/tends to favor large financial firms/i)
  })
})
