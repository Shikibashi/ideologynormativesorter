...
describe('twenty-seventh editorial pass', () => {
  it('versions and applies each normative precision rewrite', () => {
    expect(QUESTION_BANK_VERSION).toContain(EDITORIAL_TWENTY_SEVENTH_PASS_VERSION)
  ...
  it('removes the conflated mechanisms from the live wording', () => {
    expect(questionById.get('q0085')?.prompt).not.toMatch(/licensing, immigration controls, or zoning/i)
    expect(questionById.get('q0085')?.prompt).toMatch(/enter or leave employment/i)
    expect(questionById.get('q0407')?.prompt).not.toMatch(/ownership or governance/i)
    expect(questionById.get('q0407')?.prompt).toMatch(/direct governance claim/i)
  })
