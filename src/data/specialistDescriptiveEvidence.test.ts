...
describe("specialist descriptive evidence", () => {
  it("versions and operationalizes all explicitly sourced specialist descriptive items", () => {
    ...
    expect(Object.keys(specialistDescriptiveEvidenceById)).toEqual([
      "fm-fem-2",
      "fm-fem-4",
      "fm-id-13",
      "fm-an-2",
    ...
    for (const [id, evidence] of Object.entries(
      ...
      expect(
        question,
        `${id} evidence references a missing specialist item`,
      ).toBeDefined();
  ...
  it("leaves no active specialist descriptive item without an evidence scope", () => {
    const descriptive = specialistModuleDefinitions
      ...
      .filter(
        (question) =>
          question.layer === "descriptive" && question.active !== false,
      );

    expect(descriptive).toHaveLength(6);
    expect(
      descriptive.every((question) => Boolean(question.evidenceNote?.trim())),
    ).toBe(true);
    expect(
      descriptive.every((question) => (question.sources?.length ?? 0) > 0),
    ).toBe(true);
  });
});
