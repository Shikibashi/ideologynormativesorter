...
describe('tenth editorial pass', () => {
  it('separates the live religion/public-law duplicate without changing score fields', () => {
    ...
    expect(Object.keys(tenthPassRewritesById)).toEqual(['q0242', 'q0406', 'q0248', 'q0328', 'q0350', 'q0368'])
    ...
    expect(q0248.prompt).toBe(fourteenthPassRewritesById.q0248?.prompt ?? tenthPassRewritesById.q0248.prompt)
    ...
    expect(q0368.prompt).toBe(fourteenthPassRewritesById.q0368?.prompt ?? tenthPassRewritesById.q0368.prompt)
    for (const question of [q0248, q0328, q0350, q0368]) {
      expect(question.active).toBe(true)
      expect(question.version).toBe(descriptiveConstructCorrectionsById[String(question.id)]
  ...
  it('does not change the active-bank cardinality or reactivate quarantined findings', () => {
    expect(coreQuestions.filter((question) => question.active !== false)).toHaveLength(338)
