...
import {
  highPriorityLabelPrecisionFollowUps,
  labelPrecisionFollowUpById,
  labelPrecisionFollowUps,
} from "./labelPrecisionFollowUps";
...
describe("axes", () => {
  it("defines 26 axes, split 10 normative / 7 descriptive / 9 prescriptive", () => {
    ...
    const expectedPerLayer: Record<Layer, number> = {
      normative: 10,
      descriptive: 7,
      prescriptive: 9,
    };
    for (const layer of LAYERS) {
      expect(axes.filter((a) => a.layer === layer)).toHaveLength(
        expectedPerLayer[layer],
      );
    }
  });
  ...
  it("uses exclusive consumer-facing poles for corrected normative axes", () => {
    ...
    expect(antiDomination.negativePole).toMatch(
      /without continuing contestability/,
    );
    expect(antiDomination.positivePole).toMatch(
      /continuing checks, contestability/,
    );
    expect(force.negativePole).toBe(
      "Military force is never morally justified",
    );
...
describe("questions", () => {
  ...
  it("only references domains that exist", () => {
    for (const question of questions) {
      expect(
        domainById.has(question.domain),
        `${question.id} references unknown domain ${question.domain}`,
      ).toBe(true);
    }
  });
  ...
  it("only references axes that exist and share the question layer", () => {
    for (const question of questions) {
      for (const weight of question.axisWeights) {
        ...
        expect(
          axis,
          `${question.id} references unknown axis ${weight.axisId}`,
        ).toBeDefined();
        expect(
          axis!.layer,
          `${question.id} (${question.layer}) references ${weight.axisId} (${axis!.layer})`,
        ).toBe(question.layer);
      }
    }
  });
    ...
    for (const question of allQuestions) {
      for (const weight of question.axisWeights) {
        expect(
          ...
          `${question.id}/${weight.axisId} weight is not finite`,
        ).toBe(true);
        expect(
          weight.weight,
          `${question.id}/${weight.axisId} weight is below -1`,
        ).toBeGreaterThanOrEqual(-1);
        expect(
          weight.weight,
          `${question.id}/${weight.axisId} weight is above 1`,
  ...
  it("does not repeat axis ids on a question", () => {
    for (const question of allQuestions) {
      ...
      expect(new Set(axisIds).size, `${question.id} repeats an axis id`).toBe(
        axisIds.length,
      );
    }
  });
    ...
    for (const domain of domains) {
      for (const layer of LAYERS) {
        const count = questions.filter(
          (q) => q.domain === domain.id && q.layer === layer,
        ).length;
  ...
  it("has at least one domain with both an ideal and a nonideal item, to support gap scoring", () => {
    const domainsWithBoth = domains.filter((domain) => {
      ...
      return (
        items.some((q) => q.theoryContext === "ideal") &&
        items.some((q) => q.theoryContext === "nonideal")
      );
    });
  ...
  it("every descriptive item that allows dont_know provides a confidence prompt", () => {
    for (const question of questions.filter(
      (q) => q.layer === "descriptive" && q.allowDontKnow,
    )) {
      expect(
        question.confidencePrompt,
        `${question.id} is missing a confidencePrompt`,
      ).toBeTruthy();
    }
  });
  ...
  it("every prescriptive item provides a priority prompt", () => {
    for (const question of questions.filter(
      (q) => q.layer === "prescriptive",
    )) {
      expect(
        question.priorityPrompt,
        `${question.id} is missing a priorityPrompt`,
      ).toBeTruthy();
    }
  });
});
...
describe("statementChoice questions", () => {
  const statementQuestions = questions.filter(
    (q) => q.responseType === "statementChoice",
  );
  ...
  it("provides at least 3 options per item, each referencing valid same-layer axes", () => {
    for (const question of statementQuestions) {
      expect(
        question.statementOptions?.length ?? 0,
        `${question.id} needs statementOptions`,
      ...
      for (const option of question.statementOptions ?? []) {
        for (const weight of option.axisWeights) {
          ...
          expect(
            axis,
            `${question.id}/${option.id} references unknown axis ${weight.axisId}`,
          ).toBeDefined();
          expect(
            axis!.layer,
            `${question.id}/${option.id} (${question.layer}) references ${weight.axisId} (${axis!.layer})`,
          ).toBe(question.layer);
        }
      }
    }
  ...
  it("has no duplicate option ids within an item", () => {
    for (const question of statementQuestions) {
      ...
      expect(new Set(ids).size, `${question.id} has duplicate option ids`).toBe(
        ids.length,
      );
    }
  });
  ...
  it("keeps statement-option weights finite, in range, and non-repeated", () => {
    for (const question of allQuestions.filter(
      ...
      for (const option of question.statementOptions ?? []) {
        ...
        expect(
          new Set(axisIds).size,
          `${question.id}/${option.id} repeats an axis id`,
        ).toBe(axisIds.length);
        for (const weight of option.axisWeights) {
          expect(
            Number.isFinite(weight.weight),
            `${question.id}/${option.id}/${weight.axisId} weight is not finite`,
          ).toBe(true);
          expect(
            weight.weight,
            `${question.id}/${option.id}/${weight.axisId} weight is below -1`,
          ).toBeGreaterThanOrEqual(-1);
          expect(
            weight.weight,
            `${question.id}/${option.id}/${weight.axisId} weight is above 1`,
          ).toBeLessThanOrEqual(1);
        }
      }
    }
    ...
    for (const tier of FULL_COVERAGE_TIERS) {
      ...
      for (const domain of domains) {
        for (const layer of LAYERS) {
          ...
          expect(
            count,
            `${domain.id}/${layer} has no item in the ${tier} tier`,
          ).toBeGreaterThan(0);
        }
      }
    }
...
describe("labels", () => {
  ...
  it("every label centroid has exactly the known axis keys with finite in-range values", () => {
    ...
    for (const label of labels) {
      ...
      expect(
        new Set(centroidIds),
        `${label.id} centroid keys differ from axes`,
      ).toEqual(axisIdSet);
      for (const axisId of centroidIds) {
        const value = label.centroid[axisId];
        expect(
          Number.isFinite(value),
          `${label.id}/${axisId} centroid is not finite`,
        ).toBe(true);
        expect(
          value,
          `${label.id}/${axisId} centroid is below -1`,
        ).toBeGreaterThanOrEqual(-1);
        expect(
          value,
          `${label.id}/${axisId} centroid is above 1`,
        ).toBeLessThanOrEqual(1);
      }
    }
  });
  ...
  it("every centroid covers every axis", () => {
    ...
    for (const label of labels) {
      for (const axisId of axisIds) {
        expect(
          label.centroid[axisId],
          `${label.id} is missing a centroid value for ${axisId}`,
        ).toBeTypeOf("number");
      }
    }
  });
  ...
  it("every label declares a non-empty subfamily within its family", () => {
    for (const label of labels) {
      expect(typeof label.subfamily, `${label.id} is missing a subfamily`).toBe(
        "string",
      );
      expect(
        (label.subfamily ?? "").length,
        `${label.id} has an empty subfamily`,
      ).toBeGreaterThan(0);
    }
  });
  ...
  it("keeps every precision follow-up attached to an existing label", () => {
    ...
    for (const followUp of labelPrecisionFollowUps) {
      expect(
        labelIds.has(followUp.labelId),
        `${followUp.labelId} follow-up references a missing label`,
      ).toBe(true);
      expect(
        followUp.issue.length,
        `${followUp.labelId} follow-up needs an issue`,
      ).toBeGreaterThan(0);
      expect(
        followUp.recommendedAction.length,
        `${followUp.labelId} follow-up needs an action`,
      ).toBeGreaterThan(0);
    }
  });
  ...
  it("indexes precision follow-ups by label id for future UI or centroid work", () => {
    expect(labelPrecisionFollowUpById.size).toBe(
      labelPrecisionFollowUps.length,
    );
    for (const followUp of labelPrecisionFollowUps) {
  ...
  it("subTheories, if present, must be a string[]", () => {
    for (const label of labels) {
      if (label.subTheories !== undefined) {
        expect(
          Array.isArray(label.subTheories),
          `${label.id}: subTheories must be an array`,
        ).toBe(true);
        for (const st of label.subTheories) {
          expect(
            typeof st,
            `${label.id}: subTheories entry "${String(st)}" is not a string`,
          ).toBe("string");
        }
      }
    }
  ...
  it("ethicalTheory, if present, must be a string[]", () => {
    for (const label of labels) {
      if (label.ethicalTheory !== undefined) {
        expect(
          Array.isArray(label.ethicalTheory),
          `${label.id}: ethicalTheory must be an array`,
        ).toBe(true);
        for (const et of label.ethicalTheory) {
          expect(
            typeof et,
            `${label.id}: ethicalTheory entry "${String(et)}" is not a string`,
          ).toBe("string");
        }
      }
    }
    ...
    for (const label of labels) {
      ...
      for (const [field, arr] of sets) {
        if (arr !== undefined) {
          expect(
            Array.isArray(arr),
            `${label.id}: ${field} must be an array`,
          ).toBe(true);
          for (const entry of arr) {
            expect(
              philosophies.includes(entry),
              `${label.id}: ${field} "${entry}" not found in philosophies`,
            ).toBe(true);
          }
        }
      }
      ...
      for (const alias of aliases) {
        ...
        if (allNames.has(aliasLower)) {
          const otherLabel = labels.find(
            (l) => l.name.toLowerCase() === aliasLower,
          )!;
  ...
  it("philosophyInfluences entries have valid structure and axis references", () => {
    ...
    for (const label of labels) {
      ...
      expect(
        ...
        `${label.id}: philosophyInfluences must be an array`,
      ).toBe(true);
      for (let i = 0; i < influences.length; i++) {
        const entry = influences[i];
        expect(
          typeof entry.philosophy,
          `${label.id}: philosophyInfluences[${i}] philosophy is not a string`,
        ).toBe("string");
        expect(
          typeof entry.description,
          `${label.id}: philosophyInfluences[${i}] description is not a string`,
        ).toBe("string");
        expect(
          Array.isArray(entry.affectedAxes),
          `${label.id}: philosophyInfluences[${i}] affectedAxes must be an array`,
        ).toBe(true);
        for (const axisId of entry.affectedAxes) {
          expect(
            validAxisIds.has(axisId),
            `${label.id}: philosophyInfluences[${i}] references unknown axis "${axisId}"`,
          ).toBe(true);
        }
      }
    }
  ...
  it("keeps label portrayal copy descriptive rather than slogan-like", () => {
    ...
    for (const label of labels) {
      const copy = [
        label.description,
        ...(label.philosophyInfluences?.map(
      ...
      for (const pattern of disallowedPatterns) {
        expect(
          copy,
          `${label.id} uses loaded portrayal phrase ${pattern}`,
        ).not.toMatch(pattern);
      }
    }
  });
  ...
  it("keeps anti-imperialism cross-cutting rather than treating it as inherently socialist", () => {
    const antiImperialism = labels.find(
      (label) => label.id === "anti-imperialism",
    );

    expect(antiImperialism?.family).toBe("anti-colonial");
    expect(antiImperialism?.philosophies).not.toContain("Socialism");
    expect(
      antiImperialism?.philosophyInfluences?.some(
        (influence) => influence.philosophy === "Socialism",
      ),
    ).toBe(false);
  });
});
