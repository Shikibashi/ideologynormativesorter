...
const revisedCopyExpectations: Array<[string, RegExp]> = [
   ...
   ['bioregionalism', /place-based ecological political current.*local resilience/i],
   ['religious-nationalism', /Connects national identity or sovereignty with a particular religious tradition/i],
   ...
   ['anarcho-communist', /stateless communist order.*productive resources are held in common/i],
   ['minarchist', /generally permits a minimal state.*force, theft, fraud/i],
   ['objectivism', /Ayn Rand’s philosophical system/i],
   ['anarcha-feminism', /differ over which institutions must be abolished or transformed/i],
...
const bespokeContextExpectations: Array<[string, RegExp]> = [
   ...
   ['bleeding-heart-libertarianism', /liberty and market commitments.*social justice/i],
   ...
   ['universal-basic-income', /policy proposal rather than a complete ideology/i],
...
describe('label editorial review', () => {
   ...
   it('keeps bespoke context on narrow or easily conflated catalog entries', () => {
      for (const [labelId, expectedContext] of bespokeContextExpectations) {
         const label = labels.find((candidate) => candidate.id === labelId)
         expect(label, `${labelId} must exist`).toBeDefined()
         expect(`${label!.usageNote ?? ''} ${label!.cautionNote ?? ''}`, `${labelId} context regressed`).toMatch(expectedContext)
      }
   })
