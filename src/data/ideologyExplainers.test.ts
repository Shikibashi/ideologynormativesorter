...
describe("ideology explainers", () => {
  it("defines the three layers without collapsing them into one political preference", () => {
    expect(LAYER_EXPLAINERS.normative.measurement).toMatch(
      /morally legitimate/,
    );
    expect(LAYER_EXPLAINERS.descriptive.measurement).toMatch(
      /true in the world/,
    );
    expect(LAYER_EXPLAINERS.prescriptive.measurement).toMatch(
      /policies, institutions, or strategies/,
    );
    ...
    expect(LAYER_EXPLAINERS.normative.measurement).not.toBe(
      LAYER_EXPLAINERS.prescriptive.measurement,
    );
  });
  ...
  it("produces a readable layer explanation for every catalog label", () => {
    for (const label of labels) {
      for (const layer of [
        ...
        expect(summary, `${label.id}/${layer} is missing an explainer`).toMatch(
          /this layer asks/i,
        );
        expect(
          summary.length,
          `${label.id}/${layer} explainer is too long`,
    ...
    for (const label of labels) {
      for (const layer of [
        ...
        if (!summary || summary.length <= 80)
          missing.push(`${label.id}/${layer}`);
        if (summary?.match(/does not currently provide/i))
          missing.push(`${label.id}/${layer}:fallback`);
      }
    }
  ...
  it("provides a direct or pattern-matched term definition for every catalog label", () => {
    for (const label of labels) {
      ...
      expect(
        definitions.length,
        `${label.id} is missing a term definition`,
      ).toBeGreaterThan(0);
      expect(
        definitions.every((definition) => definition.length > 40),
        `${label.id} has a thin term definition`,
      ).toBe(true);
    }
  });
  ...
  it("uses the intended definitions for high-confusion ideology terms", () => {
    ...
    const cases: Array<[string, RegExp]> = [
      [
        ...
        /national continuity, inherited institutions/,
      ],
      [
        "fascist-authoritarian",
        /revolutionary ultranationalism promising national rebirth/,
      ],
      [
        "eco-fascism",
        /fascist or exclusionary ultranationalism with ecological politics/,
      ],
      [
        "strasserism",
        /radical fascist current associated with the Strasser brothers/,
      ],
      [
        "christian-democrat",
        /Christian social thought with democratic constitutionalism/,
      ],
      ["theocrat", /religious authority or binding religious doctrine/],
      ["integralism", /Catholic integralism/],
      [
        "fundamentalist-theocracy",
        /strict or literal authoritative interpretation of sacred texts/,
      ],
      ...
      [
        "socialist-feminism",
        /gender domination together with capitalism, class, labor/,
      ],
      ...
      [
        ...
        /periodic cash payment delivered individually/,
      ],
      [
        "social-investment-state",
        /building and maintaining human capabilities/,
      ],
      ...
      [
        "cultural-populism",
        /defines the people and the elite through cultural identity/,
      ],
      ...
      [
        ...
        /national self-determination or popular sovereignty/,
      ],
      [
        "expansionist-nationalism",
        /territorial enlargement, imperial influence/,
      ],
      [
        "separatist-nationalism",
        /autonomy, federal reorganization, or independence/,
      ],
      ["market-liberal", /market-oriented liberal position/],
      [
        "decentralist-market-skeptic-of-state",
        /market-liberal position that treats concentrated state power/,
      ],
      [
        "civil-libertarian-cosmopolitan",
        /civil-libertarian skepticism of concentrated authority/,
      ],
      [
        "classical-liberalism",
        /broad liberal tradition centered on individual liberty/,
      ],
      ...
      [
        "internationalism",
        /cooperation and obligations across national boundaries/,
      ],
      [
        "radical-centrism",
        /contested political style that rejects fixed left-right coalitions/,
      ],
      [
        "constitutional-monarchism",
        /hereditary monarch as head of state within constitutional rules/,
      ],
      ...
      [
        "multiculturalism",
        /family of normative views that rejects forced assimilation/,
      ],
      ...
      [
        ...
        /technology- and design-optimist environmental current/,
      ],
      ...
      [
        "communitarianism",
        /identities and moral judgments are formed through constitutive communities/,
      ],
      ...
      [
        "anarcho-syndicalism",
        /anti-state aims to syndicalist labor organization/,
      ],
      ...
      [
        "welfare-chauvinism",
        /restricting immigrants’ or other out-groups’ access/,
      ],
      ...
      [
        "anarcho-capitalist",
        /competing providers of law, protection, and arbitration/,
      ],
      ["anarcho-communist", /common control of productive resources/],
      [
        "bleeding-heart-libertarianism",
        /social justice and the interests of the least advantaged/,
      ],
      ...
      [
        ...
        /anti-capitalist and anti-authoritarian family/,
      ],
      ...
      [
        ...
        /anti-capitalist market-anarchist umbrella/,
      ],
      ...
      [
        ...
        /cross-cutting preference for using specialized knowledge/,
      ],
      [
        "black-nationalism",
        /heterogeneous traditions of Black racial consciousness/,
      ],
    ...
    for (const [id, expected] of cases) {
      ...
      expect(getIdeologyTermDefinitions(label!)).toEqual(
  ...
  it("does not require a complete ideology to have a named philosophy in every layer", () => {
    ...
    expect(getIdeologyLayerSummary(label, axes, "normative")).toContain(
      "does not currently provide a curated summary",
    );
  });
  ...
  it("does not infer term guides from rejected or comparator traditions in prose", () => {
    ...
    expect(
      getIdeologyTermDefinitions(byId.get("national-traditionalist")!),
    ).not.toEqual(expect.arrayContaining([expect.stringMatching(/Fascism/)]));
    expect(
      getIdeologyTermDefinitions(byId.get("fascist-authoritarian")!),
    ).not.toEqual(
      expect.arrayContaining([expect.stringMatching(/Liberalism/)]),
    );
    expect(
      getIdeologyTermDefinitions(byId.get("social-conservatism")!),
    ).not.toEqual(
      expect.arrayContaining([expect.stringMatching(/Progressivism/)]),
    );
    expect(
      getIdeologyTermDefinitions(byId.get("national-bolshevism")!),
    ).not.toEqual(
      expect.arrayContaining([expect.stringMatching(/Liberalism|Socialism/)]),
    );
    expect(getIdeologyTermDefinitions(byId.get("fourth-theory")!)).not.toEqual(
      expect.arrayContaining([expect.stringMatching(/Liberalism|Fascism/)]),
    );
  });

  it("keeps National Socialism separate from the generic socialism definition", () => {
    const label = labels.find(
      (candidate) => candidate.id === "national-socialism",
    )!;
    const definitions = getIdeologyTermDefinitions(label);
  ...
  it("keeps narrowed catalog labels separate from broader family definitions", () => {
    ...
    const islamicDemocracy = getIdeologyTermDefinitions(
      byId.get("islamic-democracy")!,
    );
    ...
    expect(islamicDemocracy[0]).toMatch(
      /electoral government, constitutional limits/,
    );
  ...
  it("distinguishes council, syndicalist, and platformist organization instead of showing only parent-family guides", () => {
    ...
    for (const [id, expected] of directOnlyCases) {
      ...
      expect(definitions[0], id).not.toMatch(
        /broad family of traditions|family of theories/,
      );
    }
  });
  ...
  it("distinguishes socialist traditions instead of substituting one generic socialism or Marxism guide", () => {
    ...
    expect(
      getIdeologyTermDefinitions(byId.get("libertarian-socialism")!)[0],
    ).toMatch(/distinct from right-libertarian/);
    expect(
      getIdeologyTermDefinitions(byId.get("christian-socialism")!)[0],
    ).toMatch(/disagree over markets, ownership/);
    expect(
      getIdeologyTermDefinitions(byId.get("utopian-socialism")!)[0],
    ).toMatch(/not one doctrine/);
  });
  ...
  it("distinguishes liberal and cosmopolitan variants instead of substituting generic liberalism", () => {
    ...
    const directOnlyCases: Array<[string, RegExp]> = [
      ...
      [
        "decentralist-market-skeptic-of-state",
        /distinct from socialist anarchism/,
      ],
  ...
  it("distinguishes populist variants instead of substituting the generic populism guide", () => {
    ...
    const directOnlyCases: Array<[string, RegExp]> = [
      ...
      [
        "agrarian-populism",
        /can be progressive, conservative, socialist, or pro-market/,
      ],
    ...
    for (const [id, expected] of directOnlyCases) {
      ...
      expect(definitions[0], id).toMatch(expected);
      expect(definitions[0], id).not.toMatch(
        /thin-centered|broad family of traditions|family of theories/,
      );
    }
  });
  ...
  it("distinguishes social-democratic variants instead of substituting the generic welfare guide", () => {
    ...
    const directOnlyCases: Array<[string, RegExp]> = [
      ...
      [
        "social-investment-state",
        /not a synonym for passive income maintenance/,
      ],
    ];
  ...
  it("distinguishes socialist variants instead of substituting the generic socialism guide", () => {
    ...
    const directOnlyCases: Array<[string, RegExp]> = [
      [
        ...
        /stronger ownership distinction from social democracy/,
      ],
      [
        "market-socialist",
        /differs from both private-capitalist ownership and command planning/,
      ],
  ...
  it("distinguishes authoritarian-nationalist variants instead of substituting generic authoritarianism", () => {
    ...
    const directOnlyCases: Array<[string, RegExp]> = [
      [
        "national-traditionalist",
        /distinct from ethnonationalism and fascist ultranationalism/,
      ],
      ["fascist-authoritarian", /not generic authoritarianism/],
      [
        "eco-fascism",
        /strong environmental regulation or environmental concern alone is not eco-fascism/,
      ],
  ...
  it("distinguishes community, civic, ecological, and religious-political traditions from generic family guides", () => {
    ...
    const directOnlyCases: Array<[string, RegExp]> = [
      ...
      [
        "republicanism",
        /not a contemporary party label or one regime template/,
      ],
      ["bioregionalism", /not simple localism or one economic model/],
      [
        "political-islam",
        /not synonymous with Islam, Muslim civic participation/,
      ],
    ];
  ...
  it("distinguishes monarchist forms and anti-imperialism from neighboring traditions", () => {
    ...
    const directOnlyCases: Array<[string, RegExp]> = [
      [
        "constitutional-monarchism",
        /not a synonym for traditional or absolute monarchy/,
      ],
  ...
  it("distinguishes conservative traditions instead of substituting one generic conservatism guide", () => {
    ...
    expect(getIdeologyTermDefinitions(byId.get("neoconservative")!)[0]).toMatch(
      /not simply any hawkish conservatism/,
    );
    expect(
      getIdeologyTermDefinitions(byId.get("fiscal-conservatism")!)[0],
    ).toMatch(/does not determine social or foreign policy/);
    expect(
      getIdeologyTermDefinitions(byId.get("conservative-liberalism")!)[0],
    ).toMatch(/historically variable and overlaps/);
    expect(
      getIdeologyTermDefinitions(byId.get("liberal-conservatism")!)[0],
    ).toMatch(/historically variable and overlaps/);
  });
  ...
  it("distinguishes anarchist and anti-state currents instead of substituting generic anarchism", () => {
    ...
    for (const id of directOnlyCases) {
      ...
      expect(definitions, id).toHaveLength(1);
      expect(definitions[0], id).not.toMatch(
        /subjects political authority|socialist, mutualist, individualist/,
      );
    }

    expect(
      getIdeologyTermDefinitions(byId.get("left-wing-market-anarchism")!)[0],
    ).toMatch(/not a synonym for anarcho-capitalism/);
    expect(
      getIdeologyTermDefinitions(byId.get("individualist-anarchism")!)[0],
    ).toMatch(/not a synonym for egoist anarchism/);
    expect(getIdeologyTermDefinitions(byId.get("voluntaryism")!)[0]).toMatch(
      /does not automatically mean anarchism/,
    );
    expect(getIdeologyTermDefinitions(byId.get("stirnerism")!)[0]).toMatch(
      /not ordinary selfishness/,
    );
    expect(
      getIdeologyTermDefinitions(byId.get("techno-anarchism")!)[0],
    ).toMatch(/not a synonym for blockchain advocacy/);
  });
  ...
  it("distinguishes nationalist membership and sovereignty traditions instead of substituting generic nationalism", () => {
    ...
    for (const id of directOnlyCases) {
      ...
      expect(definitions, id).toHaveLength(1);
      expect(definitions[0], id).not.toMatch(
        /broad family of traditions|subjects political authority/,
      );
    }

    expect(
      getIdeologyTermDefinitions(byId.get("civic-nationalist")!)[0],
    ).toMatch(/does not guarantee liberal democracy/);
    expect(getIdeologyTermDefinitions(byId.get("indigenism")!)[0]).toMatch(
      /not one uniform political program/,
    );
    expect(getIdeologyTermDefinitions(byId.get("hindutva")!)[0]).toMatch(
      /not the same as Hinduism as a religion/,
    );
    expect(getIdeologyTermDefinitions(byId.get("zionism")!)[0]).toMatch(
      /not a synonym for any one government/,
    );
    expect(
      getIdeologyTermDefinitions(byId.get("expansionist-nationalism")!)[0],
    ).toMatch(/distinct from nationalism in general/);
    expect(
      getIdeologyTermDefinitions(byId.get("separatist-nationalism")!)[0],
    ).toMatch(/does not by itself settle/);
  });
  ...
  it("defines theocracy and high-confusion labels directly from stable ids", () => {
    ...
    const directCases: Array<[string, RegExp]> = [
      [
        "fundamentalist-theocracy",
        /strict or literal authoritative interpretation/,
      ],
    ...
    for (const [id, expected] of directCases) {
      expect(getIdeologyTermDefinitions(byId.get(id)!)[0], id).toMatch(
        expected,
      );
    }
  });
  ...
  it("uses curated influences deterministically instead of converting centroid poles into doctrine", () => {
    const label = labels.find(
      (candidate) => candidate.id === "anarcho-capitalist",
    )!;
    const forward = getIdeologyLayerSummary(label, axes, "normative");
    const reversed = getIdeologyLayerSummary(
      label,
      [...axes].reverse(),
      "normative",
    );
  ...
  it("uses explicit layer-keyed copy where general influence notes would conflate values, beliefs, and strategy", () => {
    ...
    for (const [id, summaries] of Object.entries(
      CURATED_IDEOLOGY_LAYER_SUMMARIES,
    )) {
      const label = byId.get(id);
      expect(label, `${id} must exist`).toBeDefined();
      for (const [layer, expected] of Object.entries(summaries)) {
        expect(
          getIdeologyLayerSummary(
            label!,
            axes,
            layer as "normative" | "descriptive" | "prescriptive",
          ),
        ).toContain(expected);
      }
    }
    ...
    expect(
      getIdeologyLayerSummary(
        byId.get("national-socialism")!,
        axes,
        "normative",
      ),
    ).not.toMatch(/does not imply/i);
    expect(
      getIdeologyLayerSummary(
        byId.get("technocratic-centralist")!,
        axes,
        "descriptive",
      ),
    ).not.toMatch(/does not imply/i);
    expect(
      getIdeologyLayerSummary(byId.get("theocrat")!, axes, "prescriptive"),
    ).not.toMatch(/does not imply/i);
    expect(
      getIdeologyLayerSummary(
        byId.get("libertarian-municipalism")!,
        axes,
        "prescriptive",
      ),
    ).not.toMatch(/does not imply/i);
  ...
  it("scopes historically variable or author-specific radical-right labels", () => {
    ...
    expect(nationalBolshevism.description).toMatch(
      /post-Soviet authoritarian nationalist current/,
    );
    expect(nationalBolshevism.usageNote).toMatch(
      /distinct interwar German currents/,
    );
    expect(fourthTheory.description).toMatch(
      /does not provide one settled economic program/,
    );
    expect(fourthTheory.usageNote).toMatch(
      /claimed break.*disputes|disputes.*claimed break/,
    );
  });
  ...
  it("scopes Juche and National Bolshevism to their modeled historical reference cases", () => {
    ...
    expect(nationalBolshevism.cautionNote).toMatch(
      /historically bounded specialist label/,
    );
  });
  ...
  it("does not invent unrelated layer doctrine for broad or cross-cutting labels", () => {
    ...
    for (const [id, layer, forbidden] of cases) {
      expect(
        getIdeologyLayerSummary(byId.get(id)!, axes, layer),
        `${id}/${layer}`,
      ).not.toMatch(forbidden);
    }

    expect(
      getIdeologyLayerSummary(byId.get("voluntaryism")!, axes, "prescriptive"),
    ).toMatch(/voluntarily funded minimal state/i);
  });

  it("keeps conceptual and heterogeneous descriptive summaries qualified", () => {
    const byId = new Map(labels.map((label) => [label.id, label]));

    expect(CURATED_IDEOLOGY_LAYER_SUMMARIES.stirnerism?.descriptive).toMatch(
      /fixed ideas/,
    );
    expect(
      CURATED_IDEOLOGY_LAYER_SUMMARIES["utopian-socialism"]?.descriptive,
    ).toMatch(/model communities/);
    expect(
      getIdeologyLayerSummary(byId.get("stirnerism")!, axes, "descriptive"),
    ).toMatch(/philosophical account, not a settled empirical model/i);
    expect(
      getIdeologyLayerSummary(
        byId.get("utopian-socialism")!,
        axes,
        "descriptive",
      ),
    ).toMatch(
      /Saint-Simonian, Fourierist, and Owenite projects differed substantially/i,
    );
    expect(
      getIdeologyLayerSummary(byId.get("stirnerism")!, axes, "descriptive"),
    ).not.toMatch(/does not currently provide/i);
    expect(
      getIdeologyLayerSummary(
        byId.get("utopian-socialism")!,
        axes,
