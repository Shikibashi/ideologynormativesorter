...
describe('fifth descriptive evidence pass', () => {
  it('versions and attaches a second triangulating source to every targeted empirical item', () => {
    expect(QUESTION_BANK_VERSION).toContain(DESCRIPTIVE_EVIDENCE_FIFTH_PASS_VERSION)
    expect(Object.keys(descriptiveEvidenceFifthPassById)).toHaveLength(40)

    for (const [id, additional] of Object.entries(descriptiveEvidenceFifthPassById)) {
      ...
      expect(question?.layer, `${id} must remain descriptive`).toBe('descriptive')
      expect(question?.sources?.length, `${id} must have at least two sources`).toBeGreaterThanOrEqual(2)
      expect(question?.sources?.some((source) => source.url === additional.url)).toBe(true)
  ...
  it('leaves no active core descriptive item dependent on a single source', () => {
    ...
    expect(activeDescriptive.every((question) => (question.sources?.length ?? 0) >= 2)).toBe(true)
  })
})
