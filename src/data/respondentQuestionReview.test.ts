...
describe("respondent-facing question review", () => {
  it("applies every reviewed tier promotion before any later-pass quarantine", () => {
    for (const [questionId, promotion] of Object.entries(tierPromotionsById)) {
      ...
      expect(
        question,
        `${questionId} promotion references a missing item`,
      ).toBeDefined();
      if (
        seventhPassReplacementRequiredById[questionId] ||
        eighthPassReplacementRequiredById[questionId]
      ) {
        expect(question!.active).toBe(false);
      } else {
        expect(
          question!.active,
          `${questionId} promotion references an inactive item`,
        ).not.toBe(false);
      }
  ...
  it("quarantines replacement-required defects from public and research forms", () => {
    ...
    for (const [questionId, finding] of Object.entries(
      replacementRequiredById,
    )) {
      const question = questionById.get(questionId);
      ...
      expect(
        question,
        `${questionId} finding references a missing item`,
      ).toBeDefined();
      expect(
        raw,
        `${questionId} finding references a missing raw item`,
      ).toBeDefined();
      expect(
        question!.active,
        `${questionId} must be excluded until a replacement is reviewed`,
      ).toBe(false);
      expect(question!.reviewStatus).toBe("needs-rewrite");
      expect(question!.version).toBe(RESPONDENT_QUESTION_REVIEW_VERSION);
      const beforeRespondentReview = applyStatementSemanticReview(
        applySemanticReview(raw!),
      );
      expect(question!.axisWeights).toEqual(beforeRespondentReview.axisWeights);
      expect(
        questionsForTier(question!.tier).some((item) => item.id === questionId),
      ).toBe(false);
  ...
  it("applies neutral wording corrections without changing scoring metadata", () => {
    ...
    for (const [questionId, correction] of Object.entries(
      ...
      expect(
        raw,
        `${questionId} wording correction references a missing raw item`,
      ).toBeDefined();
  ...
  it("uses unambiguous confidence and priority prompts throughout the effective bank", () => {
    for (const question of allQuestions.filter(
      ...
      if (question.layer === "descriptive") {
        expect(question.confidencePrompt, question.id).toBe(
          DEFAULT_CONFIDENCE_PROMPT,
        );
      }
      if (question.layer === "prescriptive") {
        expect(question.priorityPrompt, question.id).toBe(
          DEFAULT_PRIORITY_PROMPT,
        );
      }
    }
  ...
  it("keeps the Blitz form cross-layer without reintroducing quarantined items", () => {
    ...
    for (const layer of LAYERS) {
      expect(
        blitz.some((question) => question.layer === layer),
        layer,
      ).toBe(true);
    }
    expect(
      blitz.every((question) => question.reviewStatus !== "needs-rewrite"),
    ).toBe(true);
  });

  it("puts two direct ecological-standing items in the short forms before Green Politics can be scored", () => {
    const blitzIds = new Set(
      questionsForTier("blitz").map((question) => question.id),
    );
    const quickIds = new Set(
      questionsForTier("quick").map((question) => question.id),
    );
  ...
  it("reports rather than conceals any Quick coverage lost to quarantine", () => {
    ...
    for (const domain of domains) {
      for (const layer of LAYERS) {
        if (
          !quick.some(
            (question) =>
              question.domain === domain.id && question.layer === layer,
          )
        ) {
    ...
    expect(
      quick.every((question) => question.reviewStatus !== "needs-rewrite"),
    ).toBe(true);
    expect(gaps).toEqual([
  ...
  it("applies statement-only deactivations to the public bank", () => {
    for (const questionId of Object.keys(statementNeedsRewriteById)) {
      ...
      expect(
        question,
        `${questionId} statement review references a missing item`,
      ).toBeDefined();
      expect(question!.active).toBe(false);
      expect(question!.reviewStatus).toBe("needs-rewrite");
