...
describe("ninth editorial pass", () => {
  it("versions the bank and rewrites the four compound prompts without changing score fields", () => {
    ...
    expect(Object.keys(ninthPassRewritesById)).toEqual([
      "q0075",
      "q0215",
      "q0274",
      "q0314",
  ...
  it("makes the two reviewed statement-choice forms explicitly ipsative", () => {
    expect(Object.keys(ninthPassStatementRewritesById)).toEqual([
      "sq02",
      "sq13",
    ]);

    for (const [id, rewrite] of Object.entries(
      ninthPassStatementRewritesById,
    )) {
      const question = questionById.get(id);
      expect(question?.reviewStatus).toBe("approved");
  ...
  it("stamps every previously unversioned active core and specialist item without erasing earlier pass history", () => {
    ...
    for (const question of [...activeCoreQuestions, ...specialistQuestions]) {
      expect(question.reviewStatus, `${question.id} review status`).toBe(
        "approved",
      );
    ...
    expect(
      activeCoreQuestions.filter(
        (question) => question.version === EDITORIAL_NINTH_PASS_VERSION,
      ),
    ).toHaveLength(82);
    expect(
      activeCoreQuestions.filter(
        (question) => question.version === EDITORIAL_TWENTIETH_PASS_VERSION,
      ),
    ).toHaveLength(2);
    expect(
      activeCoreQuestions.filter(
        (question) => question.version === EDITORIAL_TWENTY_FIRST_PASS_VERSION,
      ),
    ).toHaveLength(3);
    expect(
      activeCoreQuestions.filter(
        (question) => question.version === EDITORIAL_TWENTY_SECOND_PASS_VERSION,
      ),
    ).toHaveLength(6);
    expect(
      activeCoreQuestions.filter(
        (question) => question.version === EDITORIAL_TWENTY_THIRD_PASS_VERSION,
      ),
    ).toHaveLength(11);
    expect(
      activeCoreQuestions.filter(
        (question) => question.version === EDITORIAL_TWENTY_FOURTH_PASS_VERSION,
      ),
    ).toHaveLength(2);
    expect(
      activeCoreQuestions.filter(
        (question) => question.version === EDITORIAL_TWENTY_FIFTH_PASS_VERSION,
      ),
    ).toHaveLength(20);
    expect(
      activeCoreQuestions.filter(
        (question) =>
          question.version === EDITORIAL_TWENTY_SEVENTH_PASS_VERSION,
      ),
    ).toHaveLength(2);
    expect(
      activeCoreQuestions.filter(
    ...
    expect(
      specialistQuestions.every(
        (question) =>
          question.version === EDITORIAL_NINTH_PASS_VERSION ||
          question.version === EXPERIMENTAL_SPECIALIST_VERSION,
      ),
    ).toBe(true);
  });
});
