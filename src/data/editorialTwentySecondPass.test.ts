...
describe('twenty-second editorial pass', () => {
  it('registers and applies each source-backed correction', () => {
    expect(QUESTION_BANK_VERSION).toContain(EDITORIAL_TWENTY_SECOND_PASS_VERSION)
  ...
  it('preserves the intended layers and removes the audited shorthand', () => {
    ...
    expect(questionById.get('q0114')?.prompt).toMatch(/supply is constrained|demand/i)
    expect(questionById.get('q0123')?.prompt).toMatch(/proportionate safeguards|payment integrity/i)
    expect(questionById.get('q0142')?.prompt).toMatch(/attribution|criticism|follow-on/i)
    expect(questionById.get('q0154')?.prompt).toMatch(/shorter terms|narrower claims|exceptions/i)
    expect(questionById.get('q0158')?.prompt).toMatch(/interoperable|follow-on use/i)
    expect(questionById.get('q0217')?.prompt).toMatch(/immigration status|work authorization|due process/i)

    expect(questionById.get('q0123')?.prompt).not.toMatch(/unless specific fraud or insolvency risks are shown/i)
    expect(questionById.get('q0217')?.prompt).not.toMatch(/ordinary work or residence/i)
    expect(questionById.get('q0158')?.prompt).not.toMatch(/monopoly enforcement/i)
  })
})
