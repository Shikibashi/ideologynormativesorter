...
describe('modifier measurement registry', () => {
  it('gives every public modifier one explicit ordinary-measurement disposition', () => {
    ...
    expect(modifierMeasurementDefinitions.map((definition) => definition.labelId).sort())
    ...
    for (const labelId of MODIFIER_LABEL_IDS) {
      ...
      expect(definition, `${labelId} is missing its measurement disposition`).toBeDefined()
  ...
  it('requires direct active indicators before a modifier can be ordinarily scored', () => {
    ...
    for (const definition of modifierMeasurementDefinitions) {
      ...
      expect(definition.indicators?.length ?? 0, definition.labelId).toBeGreaterThanOrEqual(2)
      expect(definition.minimumAnsweredItems, definition.labelId).toBeGreaterThanOrEqual(2)
      expect(definition.minimumAnsweredItems, definition.labelId).toBeLessThanOrEqual(definition.indicators!.length)
      for (const indicator of definition.indicators ?? []) {
        const question = questionById.get(indicator.questionId)
        if (!question) invalidIndicators.push(`${definition.labelId}:${indicator.questionId}:unknown`)
        else if (question.active === false) invalidIndicators.push(`${definition.labelId}:${indicator.questionId}:inactive`)
        else if (question.reviewStatus === 'needs-rewrite') invalidIndicators.push(`${definition.labelId}:${indicator.questionId}:needs-rewrite`)
  ...
  it('keeps catalog and follow-up-only modifiers out of ordinary results while preserving them in the catalog', () => {
    ...
    for (const definition of modifierMeasurementDefinitions) {
      expect(catalogIds.has(definition.labelId), `${definition.labelId} should remain browsable`).toBe(true)
      if (definition.availability === 'core-construct') {
        expect(scoredIds.has(definition.labelId), `${definition.labelId} should be core scored`).toBe(true)
      } else {
        expect(scoredIds.has(definition.labelId), `${definition.labelId} must abstain from ordinary scoring`).toBe(false)
      }
    }

    expect(modifierMeasurementForLabel('ethnonationalist')?.availability).toBe('focused-follow-up')
    expect(modifierMeasurementForLabel('civic-nationalist')?.availability).toBe('catalog-only')
    expect(modifierMeasurementForLabel('nationalism')?.availability).toBe('catalog-only')
    expect(modifierMeasurementForLabel('populism')?.availability).toBe('catalog-only')
    expect(modifierMeasurementForLabel('fiscal-conservatism')?.availability).toBe('catalog-only')
  })

  it('gives every core-scored modifier enough direct indicators in the standard Balanced profile', () => {
    const balancedIds = new Set(questionsForTier('moderate').map((question) => question.id))

    for (const definition of modifierMeasurementDefinitions) {
      if (definition.availability !== 'core-construct') continue
      const balancedIndicatorCount = (definition.indicators ?? [])
        .filter((indicator) => balancedIds.has(indicator.questionId))
        .length
      expect(
