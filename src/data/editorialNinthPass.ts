...
export function isEditorialNinthPassTarget(question: Question): boolean {
  return (
    question.module === undefined ||
    specialistModules.has(String(question.module))
  );
}
...
export const ninthPassRewritesById: Readonly<Record<string, NinthPassRewrite>> =
  {
    q0075: {
      prompt:
        "Before adding new tax-funded benefits, governments should remove legal barriers that make essential goods and services unnecessarily expensive.",
      rationale:
        "Replace the earlier housing, healthcare, and work bundle with one policy-sequencing preference about supply-side barriers and tax-funded benefits.",
    },
    q0215: {
      prompt:
        "Lawful immigration pathways should expand even when housing and labor markets need time to adjust.",
      rationale:
        "Replace the earlier three-policy bottleneck bundle with one immigration-openness preference under adjustment constraints.",
    },
    q0274: {
      prompt:
        "Family policy should expand adults’ ability to exit unwanted legal or economic dependency.",
      rationale:
        "Replace the list of divorce, contraception, adoption, and work policies with the single underlying exit principle.",
    },
    q0314: {
      prompt:
        "Climate policy should reduce legal barriers to deploying low-carbon technologies and infrastructure.",
      rationale:
        "Replace the list of nuclear, transmission, housing, and innovation policies with one regulatory stance toward low-carbon deployment.",
    },
  };

export const ninthPassStatementRewritesById: Readonly<
  Record<string, NinthPassRewrite>
> = {
  sq02: {
    statementOptions: [
      {
      ...
      },
      {
        ...
        axisWeights: [
          ...
          w("equality-theory", 0.8),
        ],
      },
    ],
    rationale:
      "Separate collective governance, limited private ownership, and baseline-security priorities so the distributive options are not near-paraphrases.",
  },
  sq13: {
    prompt:
      "Which single principle should take priority when property and markets conflict?",
    statementOptions: [
      {
        id: "a",
        text: "Strong private title should be the normal baseline for productive property.",
        axisWeights: [
          w("property-legitimacy", 0.9),
          w("equality-theory", -0.4),
        ],
      },
      {
        id: "b",
    ...
    rationale:
      "Make the ipsative instruction explicit and distinguish title, anti-privilege, land, and collective-capital principles.",
  },
};
    ...
    ...(statementRewrite?.statementOptions
      ? {
          statementOptions: statementRewrite.statementOptions.map((option) => ({
            ...option,
            axisWeights: option.axisWeights.map((axisWeight) => ({
              ...axisWeight,
            })),
          })),
        }
