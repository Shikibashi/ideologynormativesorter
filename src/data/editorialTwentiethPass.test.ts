...
describe("twentieth editorial pass", () => {
  it("registers and applies each research-scoped rewrite", () => {
    ...
    expect(Object.keys(twentiethPassRewritesById)).toEqual([
      "q0007",
      "q0067",
      "q0107",
      "q0328",
  ...
  it("removes unsupported causal, universal, and conflated wording", () => {
    expect(questionById.get("q0007")?.prompt).toMatch(
      /comparisons|associated/i,
    );
    expect(questionById.get("q0007")?.prompt).not.toMatch(
      /every public service|competitive market/i,
    );
    expect(questionById.get("q0067")?.prompt).toMatch(/SNAP|recertification/i);
    expect(questionById.get("q0067")?.prompt).not.toMatch(
      /otherwise eligible recipients/i,
    );
    expect(questionById.get("q0107")?.prompt).not.toMatch(/reduce exclusion/i);
    expect(questionById.get("q0328")?.prompt).not.toMatch(
      /security, and institutional knowledge/i,
    );
    expect(questionById.get("q0402")?.prompt).not.toMatch(
      /overwhelming|before they materialize/i,
    );
    expect(questionById.get("q0402")?.contextNote).toMatch(
      /preventive war|anticipatory self-defense/i,
    );
  });
});
