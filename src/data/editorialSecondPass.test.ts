...
const review = readFileSync(
  "docs/editorial-review-second-pass-2026-08.md",
  "utf8",
);
...
describe("editorial second pass", () => {
  it("guards corrected ideology scope and taxonomy metadata", () => {
    ...
    expect(
      labelById.get("religious-nationalism")!.normativePhilosophies,
    ).not.toContain("Theocracy");

    expect(labelById.get("political-islam")!.family).toBe(
      "religious-political",
    );
    expect(labelById.get("political-islam")!.description).toMatch(
      /sometimes including Sharia-based legislation/,
    );

    expect(labelById.get("constitutional-monarchism")!.family).toBe(
      "monarchist",
    );
    expect(labelById.get("constitutional-monarchism")!.description).toMatch(
  ...
  it("keeps the retired compound out while exposing the precise specialist name", () => {
    const publicById = new Map(
      publicCatalogLabels.map((label) => [label.id, label]),
    );
    expect(publicById.get("technocratic-centralist")!.name).toBe(
  ...
  it("keeps subtype names out of aliases", () => {
    expect(labelById.get("maoism")!.aliases ?? []).not.toContain(
      "Maoism-Third Worldism",
    );
    expect(labelById.get("zionism")!.aliases ?? []).not.toContain(
      "Labour Zionism",
    );
    expect(labelById.get("strasserism")!.aliases ?? []).not.toContain(
      "National Syndicalism",
    );
    expect(
      labelById.get("libertarian-municipalism")!.aliases ?? [],
    ).not.toContain("Social Ecology");
  });

  it("keeps revised pole wording inside its declared conceptual layer", () => {
    expect(axisById.get("militarism-pacifism")!.positivePole).toMatch(
      /morally justified under specified conditions/,
    );
    expect(axisById.get("coercion-strategy")!.positivePole).toMatch(
      /^Change may be pursued/,
    );
    expect(axisById.get("anti-domination")!.negativePole).not.toMatch(
      /normal feature/,
    );
  });
  ...
  it("records resolved and empirically deferred findings in the dated review", () => {
    for (const id of [
      ...
      expect(review, `${id} is missing from the second-pass review`).toContain(
        `\`${id}\``,
      );
    }
