...
export function summarizeSpecialistEvidence(
  ...
  const constructs = Object.fromEntries(
    constructIds.map((constructId) => [constructId, {
      answeredItemCount: 0,
      itemCount: 0,
      answeredWeight: 0,
      totalWeight: 0,
  ...
  for (const evidence of Object.values(constructs)) {
    evidence.weightedCoverage = evidence.totalWeight === 0
      ? 0
      : evidence.answeredWeight / evidence.totalWeight
    evidence.effectiveItemCount = averageItemWeight === 0
      ? 0
      : evidence.answeredWeight / averageItemWeight
    evidence.sufficient = evidence.itemCount > 0 &&
      evidence.answeredItemCount >= Math.min(2, evidence.itemCount) &&
      evidence.weightedCoverage >= 0.5
  }

  const weightedAnsweredCoverage = totalWeight === 0 ? 0 : weightedAnsweredTotal / totalWeight
  const status: SpecialistEvidenceStatus = answeredItemCount === 0 || weightedAnsweredCoverage < 0.5
  ...
  return {
    ...
    effectiveItemCount: averageItemWeight === 0 ? 0 : weightedAnsweredTotal / averageItemWeight,
    status,
    ...(status === 'insufficient-evidence'
      ? { insufficientReason: 'Answer coverage is too sparse for an interpretable specialist comparison.' }
      : {}),
    constructs,
  }
}
...
export function profileEvidence(
  ...
  const coveredConstructIds = requiredConstructIds.filter((constructId) => summary.constructs[constructId]?.sufficient)
  const minimumCoveredConstructs = requiredConstructIds.length <= 1
    ? requiredConstructIds.length
    : Math.max(2, Math.ceil(requiredConstructIds.length / 2))
  const evidenceCoverage = requiredConstructIds.length === 0
    ? 0
...
export function profileDistanceConstructIds(
  ...
  return Object.keys(signals).filter((constructId) => summary.constructs[constructId]?.sufficient)
}
