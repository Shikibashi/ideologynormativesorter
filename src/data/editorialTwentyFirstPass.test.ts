...
describe('twenty-first editorial pass', () => {
  it('registers and applies each source-backed prescriptive rewrite', () => {
    expect(QUESTION_BANK_VERSION).toContain(EDITORIAL_TWENTY_FIRST_PASS_VERSION)
    expect(Object.keys(twentyFirstPassRewritesById)).toEqual(['q0135', 'q0136', 'q0318'])
  ...
  it('removes inaccurate categories and loaded shorthand', () => {
    expect(questionById.get('q0135')?.prompt).toMatch(/insured depositors|unsecured or uninsured creditors/i)
    expect(questionById.get('q0135')?.prompt).not.toMatch(/currency holders|managers/i)
    expect(questionById.get('q0136')?.prompt).toMatch(/consumer protection|payment-system stability/i)
    expect(questionById.get('q0318')?.prompt).not.toMatch(/abundance|austerity|bureaucracy/i)
    expect(questionById.get('q0318')?.contextNote).toMatch(/absolute emissions|decoupling/i)
  })
})
