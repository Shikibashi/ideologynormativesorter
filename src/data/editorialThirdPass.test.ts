...
const review = readFileSync(
  "docs/editorial-review-third-pass-2026-08.md",
  "utf8",
);
...
describe("editorial third pass", () => {
  ...
  it("keeps aliases, neighboring ideologies, policies, and research fields out of subtype lists", () => {
    expect(labelById.get("left-wing-market-anarchism")!.subTheories).toEqual([
      "Left-Rothbardianism",
    ]);
    expect(labelById.get("national-bolshevism")!.aliases).toBeUndefined();
    expect(labelById.get("integralism")!.aliases).toContain(
    ...
    expect(
      labelById.get("bleeding-heart-libertarianism")!.subTheories,
    ).toBeUndefined();
    ...
    expect(
      labelById.get("traditional-monarchist")!.subTheories,
    ).toBeUndefined();
    expect(labelById.get("civic-nationalist")!.subTheories).toEqual([
      "Liberal Nationalism",
    ]);
  });
  ...
  it("removes parent concepts, self-restatements, and filler placeholders from subtypes", () => {
    ...
    expect(radicalCentrism.subTheories).toEqual([
      "Third Way",
    ...
    for (const label of labels) {
      for (const subTheory of label.subTheories ?? []) {
        expect(
          fillerTerms.has(subTheory),
          `${label.id} retains filler subtype "${subTheory}"`,
        ).toBe(false);
      }
    }
  });
  ...
  it("records the corrected labels and unresolved question debt in the dated review", () => {
    for (const id of [
      ...
      expect(review, `${id} is missing from the third-pass review`).toContain(
        `\`${id}\``,
      );
    }
