...
describe("semantic question audit", () => {
  it("does not both correct and deactivate an item in the same semantic-review pass", () => {
    const overlap = Object.keys(semanticCorrections).filter(
      (questionId) => needsRewriteById[questionId],
    );
  ...
  it("applies every correction to an existing same-layer question", () => {
    for (const [questionId, correction] of Object.entries(
      ...
      expect(
        question,
        `${questionId} correction references a missing question`,
      ).toBeDefined();
      ...
      const twentyFifthCorrection =
        descriptiveConstructCorrectionsById[questionId];
        ...
        : twentyFifthCorrection
          ...
          : eighthPassRewrite
            ...
            : seventhPassRewrite
              ...
              : !respondentReplacement &&
                  !fifthPassReplacement &&
                  fifthPassMapping
      ...
      for (const weight of correction.axisWeights) {
        ...
        expect(
          axis,
          `${questionId} references unknown axis ${weight.axisId}`,
        ).toBeDefined();
        expect(axis!.layer, `${questionId} correction crosses layers`).toBe(
          question!.layer,
        );
  ...
  it("deactivates ambiguous items without silently assigning new weights", () => {
    for (const questionId of Object.keys(needsRewriteById)) {
      ...
      expect(
        question,
        `${questionId} review references a missing question`,
      ).toBeDefined();
      expect(question!.reviewStatus).toBe("needs-rewrite");
      expect(question!.active).toBe(false);
      ...
      expect(
        questions.some((activeQuestion) => activeQuestion.id === questionId),
      ).toBe(false);
      expect(
        questionsForTier(question!.tier).some(
          (activeQuestion) => activeQuestion.id === questionId,
        ),
      ).toBe(false);
    }
  });
  ...
  it("keeps every respondent-visible core item approved after the review overlays", () => {
    const unapproved = questions
      ...
      .map(
        (question) => `${question.id}:${question.reviewStatus ?? "missing"}`,
      );
  ...
  it("corrects known sign inversions", () => {
    ...
    expect(narrowSpeechRestrictions.axisWeights).toContainEqual({
      axisId: "coercion-strategy",
      weight: -0.8,
    });

    const competingMoney = questionById.get("q0133")!;
    expect(competingMoney.axisWeights).toContainEqual({
      axisId: "state-action-vs-exit",
      weight: -0.9,
    });
  });
});
