...
describe('research forms', () => {
  ...
  it('is stable for the same participant and administration', () => {
    const first = buildResearchQuestionForm(pool, 'p_1', 'test', 6).map((item) => item.id)
    const second = buildResearchQuestionForm(pool, 'p_1', 'test', 6).map((item) => item.id)
  ...
  it('round-robins across layer and primary-axis groups before taking second items', () => {
    ...
    const keys = new Set(form.map((item) => `${item.layer}:${item.axisWeights[0].axisId}`))
