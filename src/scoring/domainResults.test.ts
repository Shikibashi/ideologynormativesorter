...
describe("computeDomainMiniResults", () => {
  it("uses reverse-scored Likert semantics", () => {
    ...
    const result = computeDomainMiniResults(
      [question],
      { reverse: { questionId: "reverse", value: 3 } },
      [domain],
    )[0];
      ...
      statementOptions: [
        {
          id: "a",
          text: "a",
          axisWeights: [{ axisId: "norm-axis", weight: 0.2 }],
        },
        {
          id: "b",
          text: "b",
          axisWeights: [{ axisId: "norm-axis", weight: 0.4 }],
        },
        {
          id: "c",
          text: "c",
          axisWeights: [{ axisId: "norm-axis", weight: 0.6 }],
        },
        {
          id: "d",
          text: "d",
          axisWeights: [{ axisId: "norm-axis", weight: -0.75 }],
        },
      ],
    });
    const result = computeDomainMiniResults(
      [question],
      { statement: { questionId: "statement", value: 3 } },
      [domain],
    )[0];
  ...
  it("averages all applicable axis contributions for a multi-axis question", () => {
    ...
    const result = computeDomainMiniResults(
      [question],
      { multi: { questionId: "multi", value: 3 } },
      [domain],
    )[0];
  ...
  it("excludes dont_know answers", () => {
    ...
    const result = computeDomainMiniResults(
      [question],
      { unknown: { questionId: "unknown", value: "dont_know" } },
      [domain],
    )[0];
