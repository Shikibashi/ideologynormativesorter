...
describe("decomposition modules", () => {
  it("detectDivergencesAndContradictions returns reports for synthetic", () => {
    const answers = Object.fromEntries(
      questions.slice(0, 10).map((q) => [q.id, { questionId: q.id, value: 3 }]),
    );
  ...
  it("computeDomainMiniResults produces results", () => {
    const answers = Object.fromEntries(
      questions.slice(0, 5).map((q) => [q.id, { questionId: q.id, value: 0 }]),
    );
  ...
  it("computeReasonBreakdowns produces breakdowns", () => {
    const presQs = questions
      .filter((q) => q.layer === "prescriptive")
      .slice(0, 3);
    const answers = Object.fromEntries(
      presQs.map((q) => [q.id, { questionId: q.id, value: 3 }]),
    );
  ...
  it("computeReasonBreakdowns skips prescriptive dont_know answers", () => {
    ...
    expect(
      computeReasonBreakdowns(
        [question],
        { [question.id]: { questionId: question.id, value: "dont_know" } },
        [axis],
      ),
    ).toEqual([]);
  });
  ...
  it("computeReasonBreakdowns skips neutral prescriptive answers", () => {
    ...
    expect(
      computeReasonBreakdowns(
        [question],
        { [question.id]: { questionId: question.id, value: 0 } },
        [axis],
      ),
    ).toEqual([]);
  });
  ...
  it("computeReasonBreakdowns chooses the largest absolute prescriptive contribution", () => {
    ...
    const [breakdown] = computeReasonBreakdowns(
      [question],
      { [question.id]: { questionId: question.id, value: 3 } },
      syntheticAxes,
    );
