...
export function computeReasonBreakdowns(
  ...
  const presQuestions = questions.filter(
    (q) => q.layer === "prescriptive" && answers[q.id],
  );
  ...
  for (const q of presQuestions) {
    ...
    const contributions = prescriptiveAxes
      .map((axis) => ({
        axisId: axis.id,
        contribution: contributionForQuestionAxis(q, answer, axis.id),
      }))
      .filter(
        (entry): entry is { axisId: AxisId; contribution: number } =>
          entry.contribution !== null && entry.contribution !== 0,
      );
    if (contributions.length === 0) continue;

    const largestMagnitude = Math.max(
      ...contributions.map((entry) => Math.abs(entry.contribution)),
    );
