...
function directConstructUncertainty(
  ...
  if (evidenceStrength < MODIFIER_EVIDENCE_THRESHOLD || measuredItemCount < 2) return 'high'
  if (evidenceStrength < 0.7 || measuredItemCount < totalItemCount) return 'medium'
  return 'low'
}
...
export function computeDirectModifierMatches(
  ...
  const matches = labels.flatMap((label): LabelMatch[] => {
    ...
    if (definition?.availability !== 'core-construct' || !definition.indicators) return []
    ...
    for (const indicator of definition.indicators) {
      ...
      if (unit === null || (answer.salienceSkipped === true && question.layer !== 'normative')) continue
    ...
    const evidenceStrength = totalItemCount > 0 ? effectiveWeight / totalItemCount : 0
    const distance = effectiveWeight > 0
      ? Math.sqrt(squaredDistance / effectiveWeight)
      : Number.POSITIVE_INFINITY
    const fit = measuredItemCount > 0 ? closeness(distance) : 0
    const uncertaintyBand = directConstructUncertainty(evidenceStrength, measuredItemCount, totalItemCount)

    if (measuredItemCount < (definition.minimumAnsweredItems ?? totalItemCount)) return []
    ...
    return [{
      ...
      modifierConstruct: {
        ...
        answeredQuestionIds,
        indicatorQuestionIds: definition.indicators.map((indicator) => indicator.questionId),
        minimumAnsweredItems: definition.minimumAnsweredItems ?? totalItemCount,
      },
    }]
  })
