...
describe("computeLabelMatches", () => {
  it("ranks labels by ascending distance over the full axis vector", () => {
    const matches = computeLabelMatches(breakdown, [
      partialMatchLabel,
      oppositeLabel,
      exactMatchLabel,
    ]);
    expect(matches.map((m) => m.labelId)).toEqual([
      "exact-match",
      "partial",
      "opposite",
    ]);
  ...
  it("attaches reasoning breakdowns for the top matches", () => {
    const [best, partial, opposite] = computeLabelMatches(breakdown, [
      exactMatchLabel,
      partialMatchLabel,
      oppositeLabel,
    ]);
    ...
    expect(partial.reasoning?.sharedExtremeAxes.map((a) => a.axisId)).toEqual(
      expect.arrayContaining(["norm1", "norm2"]),
    );
    expect(partial.reasoning?.divergentAxes.map((a) => a.axisId)).toEqual(
      expect.arrayContaining(["presc1", "presc2"]),
    );
  ...
  it("reports independent layer evidence without changing the overall ranking", () => {
    const ranked = computeLabelMatches(
      breakdown,
      [partialMatchLabel, oppositeLabel, exactMatchLabel],
      axes,
    );
    const partial = ranked.find((match) => match.labelId === "partial");

    expect(ranked.map((match) => match.labelId)).toEqual([
      "exact-match",
      "partial",
      "opposite",
    ]);
    ...
    expect(partial?.layerEvidence?.normative).toMatchObject({
      measuredAxisCount: 2,
      totalAxisCount: 2,
    });
    expect(partial?.layerEvidence?.prescriptive).toMatchObject({
      measuredAxisCount: 2,
      totalAxisCount: 2,
    });
  });
  ...
  it("marks a layer as unmeasured instead of treating it as neutral evidence", () => {
    ...
    const [match] = computeLabelMatches(
      sparseBreakdown,
      [exactMatchLabel],
      axes,
    );
  ...
  it("ranks on a single measured axis and ignores missing axes", () => {
    ...
    const near: IdeologyLabel = {
      ...oppositeLabel,
      id: "near-single",
      name: "Near Single",
      centroid: { ...oppositeLabel.centroid, norm1: 0.8 },
    };
    const far: IdeologyLabel = {
      ...exactMatchLabel,
      id: "far-single",
      name: "Far Single",
      centroid: { ...exactMatchLabel.centroid, norm1: -0.8 },
    };
  ...
  it("uses a label scoring scope and withholds it until every required core construct is measured", () => {
    ...
    expect(
      measured[0].reasoning?.sharedExtremeAxes.map((axis) => axis.axisId),
    ).toEqual(["norm1"]);
  ...
  it("caps results at the top 20 matches", () => {
    const labels = Array.from({ length: 25 }, (_, index) => ({
      ...
      name: `Label ${index}`,
      centroid: {
        ...exactMatchLabel.centroid,
        norm1: exactMatchLabel.centroid.norm1 - index * 0.01,
      },
    }));
  ...
  it("does not penalize blitz results for axes that were not measured", () => {
    ...
    const matches = computeLabelMatches(sparseBreakdown, [
      measuredMiss,
      measuredMatch,
    ]);
...
describe("computeModifierMatches", () => {
  it("keeps only sufficiently supported modifiers and caps the public list at five", () => {
    const strongBreakdown: ScoreBreakdown = {
      normative: breakdown.normative.map((score) => ({
        ...score,
        itemCount: 3,
      })),
      descriptive: breakdown.descriptive.map((score) => ({
        ...score,
        itemCount: 3,
      })),
      prescriptive: breakdown.prescriptive.map((score) => ({
        ...score,
        itemCount: 3,
      })),
    };
    ...
    expect(matches.every((match) => match.uncertaintyBand !== "high")).toBe(
      true,
    );
  });
...
describe("computeConflatedLabels", () => {
  ...
  it("reports per-layer agreement on the native scale", () => {
    ...
    expect(flag.layerAgreement.normative).toBeGreaterThan(
      flag.layerAgreement.prescriptive,
    );
  ...
  it("does not flag a label that matches on every layer", () => {
    expect(
      computeConflatedLabels(breakdown, [exactMatchLabel], axes),
    ).toHaveLength(0);
  });

  it("does not flag a label that is far away on every layer", () => {
    expect(
      computeConflatedLabels(breakdown, [oppositeLabel], axes),
    ).toHaveLength(0);
  });
  ...
  it("does not turn a scoped label’s unmeasured layer into a false divergence", () => {
    ...
    expect(
      computeConflatedLabels(sparseBreakdown, [scopedLabel], axes),
    ).toEqual([]);
  });
});
