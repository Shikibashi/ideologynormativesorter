...
describe('synthetic centroid reflexivity sweep', () => {
   for (const fixture of allCalibrationFixtures) {
      ...
      it(`${target} remains interpretable from its full-centroid fixture`, () => {
         ...
         const own = result.nearestLabels.find((match) => match.labelId === target)
   ...
   it('keeps a synthetic fixture for every ordinary scoring label', () => {
      const covered = new Set(allCalibrationFixtures.map((fixture) => fixture.expectedLabelIds[0]))
      const uncovered = primaryScoringLabels.map((label) => label.id).filter((id) => !covered.has(id))
      expect(uncovered, `scoring labels with no synthetic coherence fixture: ${uncovered.join(', ')}`).toEqual([])
   })
})
