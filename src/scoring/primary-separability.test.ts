...
function scopedCentroidAnswers(label: IdeologyLabel): AnswerMap {
  ...
  if (!scope)
    throw new Error(`${label.id} has no source-backed primary scoring scope`);
...
function prototypeRanking(label: IdeologyLabel): string[] {
  return buildResultProfile(
    questions,
    scopedCentroidAnswers(label),
    axes,
    primaryScoringLabels,
  ).nearestLabels.map((match) => String(match.labelId));
}
  ...
  for (const label of primaryScoringLabels) {
    it(`${label.id} has measured evidence for every required core construct in its prototype`, () => {
      ...
      const own = result.nearestLabels.find(
        (match) => match.labelId === label.id,
      );

      expect(
        Object.keys(answers).length,
        `${label.id} has no effective scoped items`,
      ).toBeGreaterThan(0);
      expect(
        own,
        `${label.id} was withheld despite a complete scoped prototype`,
      ).toBeDefined();
      expect(own!.coreGateStatus).toBe("passed");
      expect(own!.measuredAxisCount).toBeGreaterThanOrEqual(
        label.scoringScope!.requiredAxisIds.length,
      );
    });
    ...
    it(`${label.id} remains within its own top-three scoped prototype neighborhood`, () => {
      ...
      expect(
        topThree,
        `${label.id} prototype resolved to ${topThree.join(", ")}`,
      ).toContain(label.id);
    });
  }
});
