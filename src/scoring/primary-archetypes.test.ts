...
function likertValue(question: Question, intent: AxisIntent): number | null {
  ...
  // Production normalization reverses these answers, so generate the raw
  // response in the opposite direction to reconstruct the intended signal.
...
describe('hand-authored primary ideology archetypes', () => {
  ...
  for (const archetype of ARCHETYPES) {
    it(`${archetype.id} resolves to the intended ideological neighborhood`, () => {
      ...
      const topFiveIds = topFive.map((match) => match.labelId)
      const topThreeFamilies = topFive
        .slice(0, 3)
        .map((match) => labelById.get(match.labelId)?.family)
      ...
      expect(
        topThreeFamilies,
        `${archetype.id} top-three families were ${topThreeFamilies.join(', ')}`,
      ).toContain(archetype.expectedFamily)
    })
  }
})