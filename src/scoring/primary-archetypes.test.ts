...
const ARCHETYPES: PrimaryArchetype[] = [
  ...
  },
  {
    ...
    targetLabelId: "market-right-libertarianism",
    expectedFamily: "liberal",
    intent: {
  ...
  },
  {
    ...
    targetLabelId: "libertarian-socialism",
    expectedFamily: "socialist",
    intent: {
  ...
  {
    id: "prudential-conservative",
    targetLabelId: "conservative",
    expectedFamily: "conservative",
    intent: {
      "authority-legitimacy": 0.2,
      "cultural-plasticity": -0.7,
...
function questionTouchesAnyAxis(
  question: Question,
  axisIds: ReadonlySet<string>,
): boolean {
  return (
    question.axisWeights.some((weight) => axisIds.has(weight.axisId)) ||
    question.statementOptions?.some((option) =>
      option.axisWeights.some((weight) => axisIds.has(weight.axisId)),
    ) === true
  );
}

function answersForIntent(
  intent: AxisIntent,
  omittedAxisIds: readonly string[] = [],
): AnswerMap {
...
describe("hand-authored primary ideology archetypes", () => {
  it("uses only primary labels in the expected taxonomy families", () => {
    for (const archetype of ARCHETYPES) {
      ...
      expect(
        target,
        `${archetype.targetLabelId} is not a primary label`,
      ).toBeDefined();
      expect(
        target!.family,
        `${archetype.targetLabelId} has the wrong taxonomy family`,
      ).toBe(archetype.expectedFamily);
    }
  });
  ...
  for (const archetype of ARCHETYPES) {
    it(`${archetype.id} resolves to the intended ideological neighborhood`, () => {
      ...
      const topFiveIds = result.nearestLabels
        .slice(0, 5)
        .map((match) => match.labelId);

      expect(
        Object.keys(answers).length,
        `${archetype.id} has too little question coverage`,
  ...
  for (const archetype of BOUNDARY_ARCHETYPES) {
    it(`${archetype.id} does not infer an unmeasured neighboring primary`, () => {
      const answers = answersForIntent(
        archetype.intent,
        archetype.omittedCoreAxisIds,
      );
      const result = buildResultProfile(
        questions,
        answers,
      ...
      expect(
        labelIds,
        `${archetype.id} should abstain from ${archetype.excludedLabelId}`,
      ).not.toContain(archetype.excludedLabelId);
    });
  }
});
