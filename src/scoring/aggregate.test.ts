...
describe("computeAxisScores", () => {
  ...
  it("shrinks a descriptive contribution toward 0 when confidence is low", () => {
    const lowConfidence: AnswerMap = {
      "q-desc-a": { questionId: "q-desc-a", value: 3, confidence: 1 },
    };
    const highConfidence: AnswerMap = {
      "q-desc-a": { questionId: "q-desc-a", value: 3, confidence: 5 },
    };
    const lowScore = computeAxisScores(questions, lowConfidence, axes).find(
      (s) => s.axisId === "desc-axis",
    )!;
    const highScore = computeAxisScores(questions, highConfidence, axes).find(
      (s) => s.axisId === "desc-axis",
    )!;
  ...
  it("shrinks a prescriptive contribution toward 0 when priority is low", () => {
    const lowPriority: AnswerMap = {
      "q-presc-a": { questionId: "q-presc-a", value: 3, priority: 1 },
    };
    const highPriority: AnswerMap = {
      "q-presc-a": { questionId: "q-presc-a", value: 3, priority: 5 },
    };
    const lowScore = computeAxisScores(questions, lowPriority, axes).find(
      (s) => s.axisId === "presc-axis",
    )!;
    const highScore = computeAxisScores(questions, highPriority, axes).find(
      (s) => s.axisId === "presc-axis",
    )!;
    expect(lowScore.raw).toBeLessThan(highScore.raw);
    expect(highScore.normalized).toBeCloseTo(1);
  ...
  it("leaves avgSalience undefined when no item provided a rating", () => {
    const answers: AnswerMap = {
      "q-presc-a": { questionId: "q-presc-a", value: 3 },
    };
    const score = computeAxisScores(questions, answers, axes).find(
      (s) => s.axisId === "presc-axis",
    )!;
  ...
  it("scores a statementChoice question using the chosen option's own axis weight", () => {
    ...
    const firstScore = computeAxisScores(questions, firstChosen, axes).find(
      (s) => s.axisId === "norm-axis",
    )!;
    const secondScore = computeAxisScores(questions, secondChosen, axes).find(
      (s) => s.axisId === "norm-axis",
    )!;
