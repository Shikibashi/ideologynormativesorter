...
describe("normalizeAnswer", () => {
  it("maps a likert7 value to -1..1", () => {
    ...
    expect(
      normalizeAnswer(question, { questionId: "q1", value: 3 }),
    ).toBeCloseTo(1);
  ...
  it("maps a likert5 value to -1..1", () => {
    ...
    expect(
      normalizeAnswer(question, { questionId: "q1", value: 2 }),
    ).toBeCloseTo(1);
    expect(
      normalizeAnswer(question, { questionId: "q1", value: -2 }),
    ).toBeCloseTo(-1);
  ...
  it("returns null for dont_know answers", () => {
    const question = makeQuestion();
    expect(
      normalizeAnswer(question, { questionId: "q1", value: "dont_know" }),
    ).toBeNull();
  });
  ...
  it("clamps out-of-range raw values", () => {
    ...
    expect(normalizeAnswer(question, { questionId: "q1", value: -99 })).toBe(
      -1,
    );
  });
});
...
describe("salienceFactor", () => {
  it("scales descriptive items by confidence", () => {
    ...
    expect(
      salienceFactor(question, { questionId: "q1", value: 1, confidence: 5 }),
    ).toBeCloseTo(1);
    expect(
      salienceFactor(question, { questionId: "q1", value: 1, confidence: 1 }),
    ).toBeCloseTo(0.2);
  });
  ...
  it("scales prescriptive items by priority", () => {
    ...
    expect(
      salienceFactor(question, { questionId: "q1", value: 1, priority: 5 }),
    ).toBeCloseTo(1);
    expect(
      salienceFactor(question, { questionId: "q1", value: 1, priority: 1 }),
    ).toBeCloseTo(0.2);
  });
  ...
  it("preserves legacy unrated answers but excludes an explicitly skipped salience rating", () => {
    ...
    expect(
      salienceFactor(normative, { questionId: "q1", value: 1, confidence: 1 }),
    ).toBe(1);
  });
});
