...
function axisWeightFor(
  ...
  return question.statementOptions?.[answer.value]?.axisWeights.find(
    (w) => w.axisId === axisId,
  );
}

export function contributionForQuestionAxis(
  question: Question,
  answer: Answer,
  axisId: AxisId,
): number | null {
...
export function computeAxisScores(
  ...
  return axes.map((axis) => {
    ...
    for (const question of questions) {
      ...
      const contribution = contributionForQuestionAxis(
        question,
        answer,
        axis.id,
      );
      ...
      const rating =
        question.layer === "descriptive" ? answer.confidence : answer.priority;
      if (rating !== undefined) {
        salienceSum += rating;
        salienceCount += 1;
      }
...
export function computeScoreBreakdown(
  ...
  return {
    normative: computeAxisScores(
      questions,
      answers,
      axes.filter((a) => a.layer === "normative"),
    ),
    descriptive: computeAxisScores(
      questions,
      answers,
      axes.filter((a) => a.layer === "descriptive"),
    ),
    prescriptive: computeAxisScores(
      questions,
      answers,
      axes.filter((a) => a.layer === "prescriptive"),
    ),
  };
}

export function axisScoreMap(
