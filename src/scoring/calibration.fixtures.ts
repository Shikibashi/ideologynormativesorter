...
export function centroidAlignmentScore(
  axisWeights: AxisWeight[],
  centroid: IdeologyLabel["centroid"],
): number | null {
  let numerator = 0;
  let denominator = 0;
...
export function statementOptionScore(
  axisWeights: AxisWeight[],
  centroid: IdeologyLabel["centroid"],
): number | null {
  let score = 0;
...
export function centroidAlignedAnswerValue(
  question: Question,
  centroid: IdeologyLabel["centroid"],
): number {
  if (question.responseType === "statementChoice") {
    ...
    if (!statementOptions) {
      throw new Error(
        `statementChoice question ${question.id} is missing statementOptions`,
      );
    }

    if (statementOptions.length === 0) {
      throw new Error(
        `statementChoice question ${question.id} has no statementOptions`,
      );
    }
    ...
    if (bestIndex < 0) {
      throw new Error(
        `statementChoice question ${question.id} has no scorable statementOptions`,
      );
    }
...
const calibrationFixtures: CalibrationFixture[] = targetIds
