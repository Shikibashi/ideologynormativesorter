...
import { modifierScoringLabels, primaryScoringLabels, publicCatalogLabels } from '../data/labelTaxonomy'
...
const civilLibertarian = modifierScoringLabels.find((label) => label.id === 'civil-libertarianism')!
const catalogOnlyPopulism = publicCatalogLabels.find((label) => label.id === 'populism')!
...
describe('direct modifier construct matching', () => {
  it('returns a modifier only from its declared direct indicators', () => {
    const matches = computeDirectModifierMatches(questions, civilLibertiesAnswers, [civilLibertarian])
  ...
  it('does not let unrelated primary-profile answers alter a direct modifier match', () => {
    const baseline = computeDirectModifierMatches(questions, civilLibertiesAnswers, [civilLibertarian])
  ...
  it('abstains below the direct-coverage minimum even when the one answered indicator is an exact fit', () => {
    expect(computeDirectModifierMatches(
      questions,
      { q0161: answer('q0161') },
      [civilLibertarian],
    )).toEqual([])
  ...
  it('uses the direct matcher on the ordinary result path', () => {
    ...
    expect(modifierMatches[0]?.modifierConstruct?.name).toBe('Civil-liberties constraint')
  })
})
