...
describe("contributionsForAxis", () => {
  ...
  it("caps at 5 and sorts by abs contribution", () => {
    const qs = Array.from({ length: 6 }, (_, i) => ({
      ...mockQuestion,
      id: "q" + i,
      axisWeights: [{ axisId: "ax1", weight: i % 2 === 0 ? 1 : -1 }],
    }));
    const answers: AnswerMap = Object.fromEntries(
      qs.map((q) => [q.id, { questionId: q.id, value: 3 }]),
    );
    const res = contributionsForAxis("ax1", qs, answers);
    expect(res.length).toBe(5);
    expect(Math.abs(res[0].contribution)).toBeGreaterThanOrEqual(
      Math.abs(res[1].contribution),
    );
  });
});
