...
describe('specialist module registry', () => {
  it('contains unique, respondent-facing modules', () => {
    ...
    for (const module of specialistModuleDefinitions) {
      ...
      expect(new Set(module.questions.map((question) => question.id)).size).toBe(module.questions.length)
    }
  })

  it('freezes the ordered assignment roster against the registered modules', () => {
    expect(SPECIALIST_ASSIGNMENT_MODULE_IDS).toEqual(specialistModuleDefinitions.map((module) => module.id))
  })

  it('versions focused module revisions separately from the experimental wave roster', () => {
    expect(specialistModuleById.get('feminist-faction-module')?.version).toBe('2026-08-v5')
    expect(specialistModuleById.get('identity-sovereignty-module')?.version).toBe('2026-08-v4')
    expect(specialistModuleById.get('monarchist-municipal-module')?.version).toBe('2026-08-specialist-v10')
  })
    ...
    for (const module of specialistModuleDefinitions) {
      for (const question of module.questions) {
        for (const weight of question.axisWeights) {
          ...
          expect(axis, `${module.id}/${question.id} references ${weight.axisId}`).toBeDefined()
          expect(axis?.layer, `${module.id}/${question.id} crosses layers`).toBe(question.layer)
        }
      }
    }
  })
  ...
  it('requires direct religious-national fusion before showing a religious-national comparison', () => {
    const module = specialistModuleById.get('religious-national-politics-module')
    expect(module).toBeDefined()

    const baseAnswers = Object.fromEntries(module!.questions.map((question) => [String(question.id), 3]))
  ...
  it('keeps the ordered specialist waves broad enough to include the new family anchors', () => {
    ...
    expect(religious?.criterionOptions.map((option) => option.traditionId)).toEqual(expect.arrayContaining([
      'religious-nationalism',
      'theocrat',
    ]))
    expect(religious?.criterionOptions.map((option) => option.traditionId)).not.toEqual(expect.arrayContaining([
    ...
    expect(religious?.constructWeightsByQuestionId['fm-rn-5']).toEqual({ 'constitutional-review': 1 })
    expect(religious?.constructWeightsByQuestionId['fm-rn-6']).toEqual({ 'party-competition': 1 })
    expect(religious?.constructWeightsByQuestionId['fm-rn-7']).toEqual({ 'islamic-public-law': 1 })
    expect(religious?.constructWeightsByQuestionId['fm-rn-8']).toEqual({ 'interpretive-pluralism': 1 })
    expect(religious?.constructWeightsByQuestionId['fm-rn-9']).toEqual({ 'hindu-civilizational-belonging': 1 })
    expect(religious?.constructWeightsByQuestionId['fm-rn-10']).toEqual({ 'jewish-national-self-determination': 1 })
    expect(religious?.constructWeightsByQuestionId['fm-rn-11']).toEqual({ 'religious-authority': 1 })
    const technology = specialistModuleDefinitions.find((module) => module.id === 'technology-governance-module')
    expect(technology?.constructWeightsByQuestionId['fm-te-5']).toEqual({ 'market-acceleration': 1 })
    expect(technology?.criterionOptions.map((option) => option.traditionId)).toEqual(expect.arrayContaining([
  ...
  it('keeps specialist layer wording aligned with the response contract', () => {
    ...
    const descriptive = questions.filter((question) => question.layer === 'descriptive')
    expect(descriptive.length).toBe(6)
    expect(descriptive.every((question) => question.allowDontKnow)).toBe(true)
    expect(descriptive.every((question) => question.confidencePrompt)).toBe(true)
    expect(descriptive.every((question) => !/\bshould\b/i.test(question.prompt))).toBe(true)

    const confederalItem = questions.find((question) => question.id === 'fm-mm-4')
    expect(confederalItem?.layer).toBe('prescriptive')
    expect(confederalItem?.prompt).toMatch(/should be coordinated/i)
    expect(confederalItem?.prompt).not.toMatch(/delegated|recallable|non-sovereign/i)
  })
