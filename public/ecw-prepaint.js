(() => {
  ...
  const appearanceValue =
    read("political-judgment-appearance-v1") ||
    read("political-judgment-theme-v1");
  const appearance = ["system", "light", "dark"].includes(appearanceValue)
    ? appearanceValue
    : "system";
  const densityValue = read("political-judgment-density-v1");
  const density = ["automatic", "compact", "comfortable"].includes(densityValue)
    ? densityValue
    : "automatic";
  const contrastValue = read("political-judgment-contrast-v1");
  const contrast = ["automatic", "standard", "more"].includes(contrastValue)
    ? contrastValue
    : "automatic";
  const theme =
    appearance === "system"
