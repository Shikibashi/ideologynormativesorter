...
`0a6ed42e361e9be7893861a65f537a3115adf37b`. The approved v13 runtime, active
...
- `src/data/vnextGraphMigration.ts` contains all 64 historical compatibility edges, each with an explicit disposition, source anchors, decision reference, rationale, and replacement edge ID. The authoritative graph contains 140 typed edges and 140 edge-level adjudication records across all 17 approved relation types. `src/data/vnextSpecialistRelationCoverage.ts` separately covers all 78 Specialist rows, representing clear ontology-node relations and dispositioning absent or ambiguous anchors with rationale.
