...
- Module overlay (`moduleSemanticAudit.ts`, `MODULE_SEMANTIC_AUDIT_VERSION=2026-07-module-semantic-v1`): 6 high-confidence construct/sign corrections (`fm-tech-1`, `fm-nat-3`, `fm-market-7`, `fm-georgist-7`, `fm-geolib-4`, `fm-christ-3`) plus 1 needs-rewrite deactivation (`fm-tech-5`) **recorded** in the authoritative module overlay module. `applyModuleSemanticReview` exists to apply this overlay, but is intentionally **not** wired into `effectiveQuestions.ts`: module questions are data-only (no module registry consumes them — see `dataValidity.test.ts` "module questions are data-only"), so the module contribution manifest remains the raw baseline (`moduleContributions` = "raw until module overlay applied"). These corrections therefore change no currently-consumed scoring path; they are staged for the eventual module-registry pipeline. Findings carry `lifecycle: 'applied'` in the sense that the overlay record was written, not that live module scoring changed.
- **Independently re-verified** (git history + live consumption trace): commit `6b3c0cf` ("Integrate module questions into main bank, remove module system entirely") deleted `factionModules.ts`/`types/factionModule.ts`. `moduleQuestions` are absent from `coreQuestions`/`questions` (only in `allQuestions`); no component, `App.tsx`, or scoring entry point imports or renders them. There is no dormant "module registry" pending activation — the feature was fully removed. `src/data/AGENTS.md` and `src/data/moduleQuestions.ts` previously described a live faction-module UI and referenced the deleted file; both corrected in this pass. The `lifecycle: 'applied'` label on module/statement findings is accurate under its documented meaning (overlay record written), and no release-report claim asserts live scoring changed.
