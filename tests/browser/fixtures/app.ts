...
export async function mockResearchEndpoint(
  page: Page,
  status = 200,
): Promise<void> {
  await page.route("**/__e2e/research", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({ accepted: status < 400 }),
    });
  });
}
...
export async function acceptContributionConsent(page: Page): Promise<void> {
  ...
  await page
    .getByRole("button", { name: "Continue to Balanced profile" })
    .click();
}
...
export async function answerCurrentQuestion(page: Page): Promise<void> {
  ...
  if (await rating.isVisible().catch(() => false))
    await rating.getByRole("button").nth(2).click();
}

export async function installStatusRecorder(page: Page): Promise<void> {
  await page.addInitScript(() => {
    const messages: string[] = [];
    Object.defineProperty(window, "__ECW_STATUS_MESSAGES__", {
      value: messages,
      configurable: true,
    });
