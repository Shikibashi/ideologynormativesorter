...
test("methodology uses browser-native history and current-page semantics", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(page.getByTestId("instrument-version")).toContainText(
    TAXONOMY_VERSION,
  );
  await expect(page.getByTestId("instrument-version")).toContainText(
    "2026-08-editorial-v16",
  );
  ...
  await expect(page).toHaveURL(/\?view=methodology$/);
  await expect(
    page.getByRole("heading", { name: "How this test works" }),
  ).toBeVisible();
});

test("modified clicks remain native and are not intercepted by application routing", async ({
  page,
}) => {
  await page.goto("/");
...
test("a shared result hash loaded on the current document opens the result workbench", async ({
  page,
}) => {
  await page.goto("/");
...
test("result fragment links update the URL, scroll, and expose current location", async ({
  page,
}) => {
  await page.goto(resultPath());
  ...
  await expect(
    page.getByRole("link", { name: "Nearest labels" }),
