...
test("intro starts a standard quiz with dynamic passive status", async ({
  page,
}) => {
  ...
  await expect(page.getByLabel("Application status")).not.toHaveAttribute(
    "aria-live",
  );
  ...
  let messages = await page.evaluate(
    () =>
      (window as unknown as { __ECW_STATUS_MESSAGES__: string[] })
        .__ECW_STATUS_MESSAGES__,
  );
  ...
  expect(
    messages.some((message) => message.includes("advanced to item 2")),
  ).toBe(true);
});
...
test("optional collection stays attached to the selected Full-depth profile", async ({
  ...
  await page
    .getByRole("checkbox", {
      name: /Optionally contribute this full-depth profile/,
    })
    .check();
  await page
  ...
  await expect(
    page.getByText("The selected profile contains 338 questions", {
      exact: false,
    }),
  ).toBeVisible();
  await page
  ...
  await expect(
    page.getByRole("progressbar", { name: "Assessment progress" }),
  ).toHaveAttribute("aria-valuemax", "338");
  await expect(page.getByLabel("Application status")).toContainText(
    "PROGRESS 1 / 338",
  );
});

test("saved session can be resumed and exposes recovery state", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Saved session available" }),
  ).toBeVisible();
  await expect(page.getByLabel("Application status")).toContainText(
    "SAVE RESUMABLE",
  );
  ...
  await expect(page.locator(".sr-status-announcer")).toContainText(
    "Resumed saved assessment progress.",
  );
});

test("contribution consent reaches the research quiz without changing refusal semantics", async ({
  page,
}) => {
  ...
  await expect(page.getByLabel("Application context")).toContainText(
    "COLLECTION community-2026-v3",
  );
  await expect(page.getByLabel("Application status")).toContainText(
    "SUBMISSION NOT SENT",
  );
  ...
  await expect(
    page.getByRole("button", { name: "Prefer not to answer" }),
  ).toBeVisible();
  await expect(page.getByLabel("Application context")).toContainText(
    "MODE CONTRIBUTION",
  );
...
test("completed contribution traverses self-identification and the specialist module", async ({
  page,
}) => {
  await installStatusRecorder(page);
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("heading", { name: "Optional specialist follow-up" }),
  ).toBeVisible();
  let messages = await page.evaluate(
    () =>
      (window as unknown as { __ECW_STATUS_MESSAGES__: string[] })
  ...
  for (let index = 0; index < 80; index += 1) {
    if (
      await page
        .getByRole("heading", { name: "Before seeing the follow-up result" })
        .isVisible()
        .catch(() => false)
    )
      break;
  ...
  await expect(
    page.getByRole("heading", { name: "Before seeing the follow-up result" }),
  ).toBeVisible();
  await page
    .getByRole("checkbox", { name: "None of these / I am not sure" })
    .check();
  await page
    .getByRole("button", { name: "Submit follow-up and show result" })
    .click();
  await expect(
    page.getByRole("heading", { name: "Experimental follow-up result" }),
  ).toBeVisible();
  messages = await page.evaluate(
    () =>
      (window as unknown as { __ECW_STATUS_MESSAGES__: string[] })
...
test("completion, share-copy, and comparison events are announced discretely", async ({
  page,
  context,
}) => {
  await installStatusRecorder(page);
  ...
  await page
    .getByRole("textbox", { name: "Shared result link to compare" })
    .fill(compareUrl);
  await page.getByRole("button", { name: "Compare" }).click();
  await expect(
    page.getByRole("heading", { name: "Comparison view" }),
  ).toBeVisible();
...
test("shared results, comparison, and the label browser are addressable", async ({
  page,
}) => {
  ...
  await expect(scoredCard).toContainText(
    /primary scored family|modifier scored independently/,
  );
  ...
  expect(
    await scoredCard.locator(".label-source-disclosure a").count(),
  ).toBeGreaterThan(0);
  await page.locator("details.full-label-browser > summary").click();
  const labelBrowser = page.locator("details.full-label-browser");
  await expect(
    labelBrowser.getByRole("heading", {
      name: "Browse the public label catalog",
    }),
  ).toBeVisible();
  const search = page.getByRole("searchbox", {
    name: "Search ideology labels",
  });
  await expect(search).toBeVisible();
  await expect(
    labelBrowser.getByText("Libertarian Leaning", { exact: true }),
  ).toHaveCount(0);

  await search.fill("Anarcho-Capitalist");
  await expect(
    labelBrowser.getByRole("heading", { name: "Anarcho-Capitalist" }),
  ).toBeVisible();
  await expect(labelBrowser.locator("summary.family-name")).toContainText(
    "Anarchist",
  );
  await expect(
    labelBrowser.getByText("provisional specialist", { exact: true }),
  ).toBeVisible();
  ...
  await expect(
    labelBrowser.getByText("Socialist", { exact: true }),
  ).toBeVisible();
  await expect(
    labelBrowser.getByRole("heading", { name: "Related traditions" }),
  ).toHaveCount(0);

  await search.fill("Nyerereism");
  await expect(
    labelBrowser.getByRole("heading", { name: "Ujamaa / Nyerereism" }),
  ).toBeVisible();
  ...
  await expect(
    labelBrowser.getByText("Sources for this catalog summary"),
  ).toBeVisible();
  await expect(labelBrowser.locator(".label-source-disclosure a")).toHaveCount(
    2,
  );
});
