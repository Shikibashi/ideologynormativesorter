...
test('shared results, comparison, and the label browser are addressable', async ({ page }) => {
  ...
  await expect(labelBrowser.getByText('Sources for this catalog summary')).toBeVisible()
  await expect(labelBrowser.locator('.label-source-disclosure a')).toHaveCount(2)
})
