...
test("malformed share link identifies the object, problem, and recovery action", async ({
  page,
}) => {
...
test("local save failure stays visible and is announced", async ({ page }) => {
  ...
  await expect(page.locator(".section-band-status")).toContainText(
  ...
  expect(
    messages.some((message) => message.includes("storage is full or disabled")),
  ).toBe(true);
});

test("failed contribution is retained locally and restored after reload", async ({
  page,
}) => {
  await installStatusRecorder(page);
...
test("saved-session reset confirmation preserves keyboard focus", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Clear saved session" }),
  ).toBeFocused();
