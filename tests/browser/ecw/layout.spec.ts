  ...
  const result = await page.evaluate(() => {
    ...
    const clipped = [
      ...document.querySelectorAll<HTMLElement>(
        "h1, h2, p, button, label, .section-band",
      ),
    ]
      .filter((element) => {
...
for (const viewport of viewports) {
  test(`reflows Page and Workbench at ${viewport.width}x${viewport.height}`, async ({
    page,
  }) => {
...
test("Workbench container independently demotes through three, two, and one columns", async ({
  page,
}) => {
  ...
  const focusOrder = await page
    .locator(".results-navigator a")
    .allTextContents();
  const semanticOrder = await page.evaluate(() => {
    const profile = document.querySelector("#profile");
    const comparison = document.querySelector(
      ".compare-input-area, .compare-banner",
    );
    const labels = document.querySelector("#labels");
    return {
      profileBeforeComparison: Boolean(
        profile &&
          comparison &&
          profile.compareDocumentPosition(comparison) &
            Node.DOCUMENT_POSITION_FOLLOWING,
      ),
      profileBeforeLabels: Boolean(
        profile &&
          labels &&
          profile.compareDocumentPosition(labels) &
            Node.DOCUMENT_POSITION_FOLLOWING,
      ),
    };
  });
  expect(semanticOrder).toEqual({
    profileBeforeComparison: true,
    profileBeforeLabels: true,
  });

  await screen.evaluate((element) => {
    (element as HTMLElement).style.width = "1300px";
  });
  let [navRect, primaryRect, inspectorRect] = await Promise.all([
  ...
  const compareRect = await page
    .locator(".compare-input-area, .compare-banner")
    .boundingBox();
  expect(Math.abs((compareRect?.y ?? 0) - (primaryRect?.y ?? 0))).toBeLessThan(
    2,
  );
  ...
  [navRect, primaryRect, inspectorRect] = await Promise.all([
    navigator.boundingBox(),
    primary.boundingBox(),
    inspector.boundingBox(),
  ]);
  ...
  expect(Math.abs(inspectorRect!.x - primaryRect!.x)).toBeLessThan(2);
  expect(await page.locator(".results-navigator a").allTextContents()).toEqual(
    focusOrder,
  );
  await expectNoConcealedOverflow(page);
});

test("Result cards remain readable and the nearest-label inspector stays bounded", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1920, height: 1080 });
  await page.goto(resultPath());
  ...
  const metrics = await page.evaluate(() => {
    const axisNames = [
      ...document.querySelectorAll<HTMLElement>(".axis-bar-name"),
    ].map((element) => {
  ...
  expect(
    Math.min(...metrics.axisNames.map(({ width }) => width)),
  ).toBeGreaterThan(240);
  expect(
    Math.max(...metrics.axisNames.map(({ height }) => height)),
  ).toBeLessThan(64);
