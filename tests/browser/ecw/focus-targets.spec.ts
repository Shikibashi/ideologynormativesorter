...
async function expectTwoBandFocus(
  locator: Locator,
  moveFocus = true,
): Promise<void> {
...
async function expectVisibleTargetsMeetFloor(
...
test("links, buttons, fields, and Display receive canonical two-band focus", async ({
  page,
}) => {
  await page.goto("/");
  await expectTwoBandFocus(page.getByRole("link", { name: "METHODOLOGY" }));
  await expectTwoBandFocus(
  ...
  await expectTwoBandFocus(
    page.getByRole("textbox", { name: "Shared result link to compare" }),
  );
  await expectTwoBandFocus(page.getByRole("link", { name: "Profile" }));
  await expectTwoBandFocus(
    page.locator("details.full-label-browser > summary"),
  );
});

test("selected answer and tier states remain visible while focused", async ({
  page,
}) => {
  await page.goto("/");
...
test("computed ECW targets meet Compact and Comfortable geometry", async ({
  page,
}) => {
  await page.goto("/");
  await expectVisibleTargetsMeetFloor(page);
  const compactHeight =
    (await page.getByRole("button", { name: "Begin assessment" }).boundingBox())
      ?.height ?? 0;
...
test("Dark mode keeps keyboard focus visible on Page and Workbench controls", async ({
  page,
}) => {
...
test("forced colors preserve selected focus and structural borders", async ({
  page,
}) => {
