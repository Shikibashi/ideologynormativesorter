...
test("intro, methodology, consent, and standard quiz pass automated WCAG checks", async ({
  page,
}, testInfo) => {
  await page.goto("/");
...
test("representative Page and Workbench stages pass automated checks in Dark mode", async ({
  page,
}, testInfo) => {
...
test("saved resume, results, comparison, and label browser pass automated WCAG checks", async ({
  page,
}, testInfo) => {
...
test("self-identification and specialist module pass automated WCAG checks", async ({
  page,
}, testInfo) => {
...
test("passive status and polite announcements remain separate without save chatter", async ({
  page,
}) => {
  ...
  await expect(page.locator(".sr-status-announcer")).toHaveAttribute(
    "aria-live",
    "polite",
  );
  ...
  expect(
    messages.filter(
      (message) => message === "Assessment progress saved locally.",
    ),
  ).toHaveLength(1);
  expect(messages.some((message) => message.includes("advanced to item"))).toBe(
    true,
  );
});

test("selected controls expose state and compare errors are associated", async ({
  page,
}) => {
  await page.goto(resultPath());
  const compareInput = page.getByRole("textbox", {
    name: "Shared result link to compare",
  });
  await compareInput.fill("not a shared link");
  await page.getByRole("button", { name: "Compare" }).click();
  await expect(compareInput).toHaveAttribute("aria-invalid", "true");
  await expect(compareInput).toHaveAttribute(
    "aria-describedby",
    "compare-error",
  );
